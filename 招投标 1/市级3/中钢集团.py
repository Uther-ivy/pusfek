...
class alashan_ggzy:
    def __init__(self):
        ...
        self.domain_name='http://tendering.sinosteel.com'
    ...
    def parse(self):
        ...
        while True:
            page += 1
            url=f'http://tendering.sinosteel.com/bidding/cid-16/page{page}/'
            print('*' * 20, page, '*' * 20)
            text = tool.requests_get(url, self.headers).replace('&#x2f;', '/')
            html = HTML(text)
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath("//ul[@class='m2zbul clearfix wow fadeInUp']/li")
            for li in detail:
                title = li.xpath('./a/strong/text()')[0].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                url = li.xpath('./a/@href')[0]
                date_Today = li.xpath("./a/h6/text()")[0].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '').replace('发布时间：','')
                if 'http' not in url:
                    url = self.domain_name + url
                    print(title, url, date_Today)
                print( tool.Transformation(date) ,tool.Transformation_zhong(date_Today))
                # time.sleep(666)
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath("//div[@class='container max1240 wow fadeInUp']")[0]
        # print(url_text)
        ...
        if item['endtime'] == '':
            item['endtime'] = int(time.mktime(time.strptime(date, "%Y年%m月%d日")))
        else:
            try:
                item['endtime'] = int(time.mktime(time.strptime(item['endtime'], "%Y年%m月%d日")))
            except:
