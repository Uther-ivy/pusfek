...
class alashan_ggzy:
    def __init__(self):
        ...
        self.domain_name='http://bulletin.sntba.com'
    ...
    def parse(self):
        ...
        while True:
            page += 1
            url=f'http://bulletin.sntba.com/xxfbcmses/search/bulletin.html?dates=300&categoryId=88&page={page}&showStatus=1'
            print('*' * 20, page, '*' * 20)
            text = tool.requests_get(url, self.headers).replace('&#x2f;', '/')
            html = etree.HTML(text)
            # print(11, text)
            detail = html.xpath("//tr/td/a")
