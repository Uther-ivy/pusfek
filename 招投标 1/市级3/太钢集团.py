...
class alashan_ggzy:
    def __init__(self):
        ...
        self.domain_name='https://bid.tisco.com.cn'
    ...
    def parse(self):
        ...
        while True:
            page += 1
            if page==1:
                url='https://bid.tisco.com.cn/jyxx/007001/level4.html'
            else:
                url = f'https://bid.tisco.com.cn/jyxx/007001/{page}.html'
            print('*' * 20, page, '*' * 20)
            text = tool.requests_get(url, self.headers).replace('&#x2f;', '/')
            html = HTML(text)
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath("//ul[@id='info']/li")
            for li in detail:
                title = li.xpath('./a/text()')[0].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                url = li.xpath('./a/@href')[0]
                date_Today = li.xpath("./span[@class='em-time']/text()")[0].replace('\xa0', '').replace('\n', ''). \
