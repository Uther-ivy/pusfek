...
class alashan_ggzy:
    def __init__(self):
        ...
        self.domain_name='https://e.sinochemitc.com'
    ...
    def parse(self):
        ...
        while True:
            page += 1
            url=f'https://e.sinochemitc.com/cms/channel/ywgg1qb/index.htm?pageNo={page}'
            ...
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath("//ul[@class='search-list']/li")
            for li in detail:
                title = li.xpath('./a/text()')[0].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                url = li.xpath('./a/@href')[0]
                date_Today = li.xpath("./p[@class='time']/text()")[0].replace('\xa0', '').replace('\n', ''). \
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath("//div[@class='a-content']")[0]
        # print(url_text)
