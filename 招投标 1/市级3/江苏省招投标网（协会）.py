...
class wenshan_ggzy:
    def __init__(self):
        self.domain_name = 'https://api.jszbtb.com'
        self.url_list = [
    ...
    def parse(self):
        ...
        while True:
            page += 1
            url=f'https://api.jszbtb.com/DataSyncApi/HomeTenderBulletin?CurrentPage={page}&PageSize=10&only=1'

            text = tool.requests_get(url , self.headers)
            print('*' * 20, page, '*' * 20)
            # print(11, text.replace('\ufffd', ' ').replace('\u30fb', ' '))
            # time.sleep(6666)
            detail = json.loads(text)['data']['data']
            for li in detail:
                title = li['bulletinName']
                pid= li['id']
                url = f'http://www.jstba.org.cn/5_2.html?id={pid}&type=bids&typeid=36'
    ...
    def parse_detile(self, title, url, date,pid):
        print(url)
        jsonurl=f'https://api.jszbtb.com/DataSyncApi/TenderBulletin/id/{pid}'

        t = tool.requests_get(jsonurl, self.headers).replace('\xa9', '').replace('<br>&nbsp;&nbsp;', '')\
            .replace('</o:p><', '').replace('<o:p><', '')
        data=json.loads(t)['data']['data']
        # print(data)
        # time.sleep(2222)
        detail = etree.HTML(data[0]['bulletincontent'])
