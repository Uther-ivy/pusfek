...
class baoshan_ggzy:
    ...
    def parse(self):
        ...
        while True:
            page += 1
            data = f"page={page}&TimeStr=&allDq=&allHy=reset1%2C&AllPtName=&KeyStr=&KeyType=ggname"
            text = tool.requests_post1(self.url.format(page), headers=self.headers, data=data)
            print('*' * 20, page, '*' * 20)

            html = HTML(text)
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath('//div[@class="publicont"]//h4')
            for li in detail:
                title = li.xpath('./a/text()')[0].replace('\n', '').replace('\t', '').replace(' ', '').replace('\r',
                                                                                                                '')
                url = li.xpath('./a/@href')[0]
                print(url)
                url = f'http://www.hebeieb.com/'+url

                date_Today = li.xpath('./span[@class="span_o"]/text()')[0].split(' ')[0]
                if '测试' in title:
                    continue
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//table[@class="infro_table"]|//div[@class="article_title"]')[0]
