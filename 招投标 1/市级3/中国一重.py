...
class alashan_ggzy:
    def __init__(self):
        ...
        self.domain_name='http://jzcg.cfhi.com'
    ...
    def parse(self):
        ...
        while True:
            page += 1
            if  page==1:
                url='http://jzcg.cfhi.com/zbgg/index.jhtml'
            else:
                url=f'http://jzcg.cfhi.com/zbgg/index_{page}.jhtml'
            ...
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath("//div[@class='BorderEEE NoBorderTop List1 Black14 Padding5']/ul/li")
            for li in detail:
                title = li.xpath('./a/text()')[0].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                url = li.xpath('./a/@href')[0]
                date_Today = li.xpath("./span[@class='Right Gray']/text()")[0].replace('\xa0', '').replace('\n', ''). \
