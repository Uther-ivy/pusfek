...
class wenshan_ggzy:
    def __init__(self):
        self.domain_name = 'http://www.ccgp-liaoning.gov.cn'
        self.url_list = [
    ...
    def parse(self):
        ...
        while True:
            page += 1
            url=f'http://www.ccgp-liaoning.gov.cn/portalindex.do?method=getPubInfoList&t_k=null&tk=0.15036367696899444'
            payload = {
            'current': 1,
            'rowCount': 50,
            'searchPhrase':'',
            ...
            for li in detail:
                title = li['title']
                cid= li['id']
                url = f'http://www.ccgp-liaoning.gov.cn/portalindex.do?method=getPubInfoViewOpenNew&infoId={cid}'
                date_Today = li['tovStart']
                if 'http' not in url:
    ...
    def parse_detile(self, title, url, date,endtime):
        ...
        html_=etree.HTML(t)
        data=html_.xpath('//div[@id="template"]/text()')[0]
        print(data)
        # time.sleep(2222)
        detail = etree.HTML(data)
