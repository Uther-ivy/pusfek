...
class baoshan_ggzy:
    ...
    def parse(self):
        ...
        while True:
            page += 1
            data = '{"pubTime":"","tradeType":"","regionCode":"","afficheSourceType":"","afficheTitle":"","pageNum":'+str(page)+',"pageSize":10}'
            ...
            # print(11, text)
            #
            detail = html.xpath('//div[@class="titBox"]')
            for li in detail:
                title = li.xpath('./div[@class="fl tit"]/a/text()')[0].replace('\n', '').replace('\t', '').replace(' ', '').replace('\r', '')

                date_Today = li.xpath('./div[@class="fr nums"]/text()')[0].split(' ')[0]
                url = li.xpath('./div[@class="fl tit"]/a/@href')[0]
                print(url)
                url = f'http://www.ahtba.org.cn/'+url
    ...
    def parse_detile(self, title, url, date):
        print(url)
        jsonurl = f'http://www.ahtba.org.cn/htmlUrl/trade_/{date}/{url.split("/")[-1]}.html'
        print(jsonurl)
        t = tool.requests_get(jsonurl, self.headers)
        url_html = etree.HTML(t)
        detail = url_html.xpath("//div[@class='detailCon']/div")[0]
        print(detail)
