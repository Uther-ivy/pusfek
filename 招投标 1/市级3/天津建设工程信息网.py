...
class baoshan_ggzy:
    def __init__(self):
        self.url_list = [
            'http://www.tjconstruct.cn/Zbgg/Index/{}'
        ]
    ...
    def parse(self):
        ...
        while True:
            ...
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath('//table[@class="t1"]//tr')[2:]
            for li in detail:
                title = li.xpath('./td[1]/a/text()')[0].replace('\n', '').replace('\t', '').replace(' ', '').replace('\r',
                                                                                                       '')
                url = li.xpath('./td[1]/a/@href')[0]

                url = f'http://www.tjconstruct.cn/'+url

                date_1 = li.xpath('./td[4]/text()')[0].replace(' ','').replace('\t','').replace('\n','').replace('\r','').split('/')
                date_Today=date_1[0]+'-'+date_1[1]+'-'+date_1[2]
                print(date_Today)
                if '测试' in title:
                    continue
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//div[@class="body"]')[0]
