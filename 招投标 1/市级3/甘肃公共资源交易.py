...
class baoshan_ggzy:
    def __init__(self):
        self.url_list = [
          'https://pzxx.ggzyjy.gansu.gov.cn/f/newprovince/annogoods/getAnnoList?pageNo={}&pageSize=10&area=620000&prjpropertynewI=I&prjpropertynewA=A&prjpropertynewD=D&prjpropertynewC=C&prjpropertynewB=B&prjpropertynewE=E&prjpropertynewZ=Z&projectname='
        ]
    ...
    def parse(self):
        ...
        while True:
            ...
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath('//div[@class="sTradingInformationSelectedBtoList"]/dl')
            for li in detail:
                title = li.xpath('./dd/p/a/@title')[0].replace('\n', '').replace('\t', '').replace(' ', '').replace('\r', '')
                projectid=li.xpath('./dd/p/a/@onclick')[0].split(',')[0].replace("loadTender('",'').replace("'",'')
                url = "https://pzxx.ggzyjy.gansu.gov.cn/f/newprovince/tenderproject/flowpage?bidpackages=&tenderprojectid=%s&index=0&area=620000"%projectid
                url_domain = 'https://pzxx.ggzyjy.gansu.gov.cn/'
                if 'http' not in url:
                    if '../../' in url:
