...
class alashan_ggzy:
    def __init__(self):
        ...
        self.domain_name='http://www.chinagoldgroup.com'
    ...
    def parse(self):
        ...
        while True:
            page += 1
            url=f'http://www.chinagoldgroup.com/hjjs/l/2110-6658-{page}.html'
            print('*' * 20, page, '*' * 20)
            text = tool.requests_get(url, self.headers).replace('&#x2f;', '/')
            html = HTML(text)
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath("//div[@class='second-item']")
            for li in detail:
                title = li.xpath('.//a/text()')[0].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                url = li.xpath('.//a/@href')[0]
                date_Today = li.xpath(".//span[@class='currentdate']/text()")[0].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                if 'http' not in url:
