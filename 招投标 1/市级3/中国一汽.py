...
class alashan_ggzy:
    def __init__(self):
        ...
        self.domain_name='https://etp.faw.cn'
    ...
    def parse(self):
        ...
        while True:
            page += 1
            url=f'https://etp.faw.cn/gg/allJYTypeGGList?hangYeType=-1&xmLeiXing=&ggStartTimeEnd=&gongGaoType=5&isNew=1'
            data={
                'searchType': '',
                'searchText': '',
                'currentPage': page
            ...
            print('*' * 20, page, '*' * 20)
            text = requests.session().post(url,params=data, headers=self.headers).content.decode().replace('&#x2f;', '/')
            html = HTML(text)
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath("//div[@class='zl-col-6']")
            for li in detail:
                title = li.xpath(".//div[@class='title']/text()")[0].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                gid = re.findall(r"\('\d',\'([\w\d\-]+\:\d)\',",li.xpath('./div/@onclick')[0])
                url=f'https://etp.faw.cn/gg/toGongGaoDetail?guid={gid[0]}&gongGaoType=6&&isNew=1'
                date_Today = li.xpath(".//div[@class='zl-desc-item']/text()")[0].replace('\xa0', '').replace('\n', ''). \
    ...
    def parse_detile(self, title, url, date):
        ...
        # print(url_text)
        detail = url_html.xpath("//div[@class='neirong']")[0]
