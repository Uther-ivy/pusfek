...
class alashan_ggzy:
    def __init__(self):
        ...
        self.domain_name='http://ecp.cnmc.com.cn'
    ...
    def parse(self):
        ...
        while True:
            page += 1
            url=f'http://ecp.cnmc.com.cn/zgyscms/category/bulletinList.html?searchDate=1997-12-09&dates=300&goSearch=&categoryId=2&tabName=&projectType=&itemId=&purType=00,01&startPublishDate=&endPublishDate=&page={page}'
            print('*' * 20, page, '*' * 20)
            text = tool.requests_get(url ,self.headers).replace('&#x2f;', '/')
            html = HTML(text)
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath("//tbody[@id='bulletinList']/tr")
            for li in detail:
                title = li.xpath('./td[1]/a/text()')[0].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                url = li.xpath('.//input/@value')[0]
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath("//div[@class='contant']")[0]#//iframe[@id='pdfContainer']
        if detail.xpath("//iframe[@id='pdfContainer']/@src"):
            pdf_url= detail.xpath("//iframe[@id='pdfContainer']/@src")[0]
            detail = f'<a href ={pdf_url}>{title}.pdf</a>'
            # print(detail)
