...
class alashan_ggzy:
    def __init__(self):
        ...
        self.domain_name='https://bid.wuliangye.com.cn'
        self.url ='https://bid.wuliangye.com.cn/gg/ggList'
        self.headers = {
    ...
    def parse(self):
        ...
        while True:
            ...
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath("//table[@id='p2']//tr")
            print(len(detail))
            for li in range(2,len(detail)+1):
                title = html.xpath(f'//tr[{li}]//a/text()')[0].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                url = html.xpath(f'//tr[{li}]//a/@href')[0]
                date_Today = html.xpath(f"//tr[{li}]/td[4]/text()")[0].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                if 'http' not in url:
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//form[@id="xmForm"]|//div[@class="template"]|//div[@class="neirong"]')[0]
