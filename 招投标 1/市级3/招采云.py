...
class wenshan_ggzy:
    def __init__(self):
        self.domain_name = 'http://www.zcbidding.com'
        self.url_list = [
    ...
    def parse(self):
        ...
        while True:
            ...
            text = requests.session().post(url,self.headers,params=payload).content.decode()
            print('*' * 20, page, '*' * 20)
            ...
            for li in detail:
                title = li['title']
                cid= li['id']
                url = f'http://www.zcbidding.com/portal/ggXq?id={cid}'
                date_Today = li['createdAt']
                content=li['descr']
                if 'http' not in url:
