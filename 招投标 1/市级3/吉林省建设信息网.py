...
class alashan_ggzy:
    def __init__(self):
        ...
        self.domain_name='http://www.jlsjsxxw.com'
    ...
    def parse(self):
        ...
        while True:
            page += 1
            if page==1:
                url='http://www.jlsjsxxw.com/gcxx/004001/moreinfojyxx.html'
            else:
                url = f'http://www.jlsjsxxw.com/gcxx/004001/{page}.html'
            print('*' * 20, page, '*' * 20)
            text = tool.requests_get(url, self.headers).replace('&#x2f;', '/')
            html = HTML(text)
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath("//tbody[@id='list']/tr")
            for li in detail:
                title = li.xpath("./td[@class='ewb-center ewb-w40']/a/text()")[0].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                url = li.xpath("./td[@class='ewb-center ewb-w40']/a/@href")[0]
                date_Today = li.xpath("./td[@class='ewb-center ewb-w15'][2]/span/text()")[0].replace('\xa0', '').replace('\n', ''). \
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath("//div[@class='infor-con']")[0]
        # print(url_text)
