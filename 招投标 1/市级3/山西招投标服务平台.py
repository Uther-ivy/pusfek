...
class baoshan_ggzy:
    ...
    def parse(self):
        ...
        while True:
            page += 1
            data=f"pageNo={page}&pageSize=15&title=&recentType="
            ...
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath('//table[@class="content_table"]/tbody/tr')
            for li in detail:
                title = li.xpath('./td[1]/a/@title')[0].replace('\n', '').replace('\t', '').replace(' ', '').replace('\r', '')
                url = li.xpath('./td[1]/a/@href')[0]
                print(url)
                url = f'https://www.sxbid.com.cn'+url

                date_Today = li.xpath('./td[3]/text()')[0].split(' ')[0]
                if '测试' in title:
                    continue
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//div[@class="sxca_container"]')[0]
