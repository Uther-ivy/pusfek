...
class alashan_ggzy:
    def __init__(self):
        ...
        self.domain_name='https://zfcg.czt.fujian.gov.cn'
    ...
    def parse(self):
        ...
        while True:
            page += 1
            url=f'https://zfcg.czt.fujian.gov.cn/freecms/rest/v1/notice/selectInfoForIndex.do?&siteId=d36a6e8b-4363-4b52-a00b-79ca47033923&channel=f582600e-065d-4f35-8966-48a33fa93863&currPage=1&pageSize=9&noticeType=001051&regionCode=!350001&cityOrArea='
            print('*' * 20, page, '*' * 20)
            text = tool.requests_get(url, self.headers).replace('&#x2f;', '/')
            # print(11, text)
            # time.sleep(6666)
            for li in json.loads(text).get('data'):
                title =li.get('title')
                url = li.get('htmlpath')
                date_Today = li.get("noticeTime")
                if 'http' not in url:
                    url = self.domain_name +'/freecms'+ url+'?noticeType=001051'
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath("//div[@class='noticeArea']")[0]
        # print(url_text)
