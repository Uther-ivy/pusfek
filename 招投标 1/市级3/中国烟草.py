...
class alashan_ggzy:
    def __init__(self):
        ...
        self.domain_name='http://www.tobacco.gov.cn'
    ...
    def parse(self):
        ...
        while True:
            page += 1
            if page==1:
                url='http://www.tobacco.gov.cn/gjyc/zfcg/list.shtml'
            else:
                url = f'http://www.tobacco.gov.cn/gjyc/zfcg/list_{page}.shtml'
            print('*' * 20, page, '*' * 20)
            text = tool.requests_get(url, self.headers).replace('&#x2f;', '/')
            html = HTML(text)
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath("//ul[@class='inTyList']/li")
            for li in detail:
                title = li.xpath('./a/text()')[0].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                url = li.xpath('./a/@href')[0]
                date_Today = li.xpath("./span[@class='date fr']/text()")[0].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                if 'http' not in url:
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath("//div[@class='ldConRMain tyContentMain']")[0]
        # print(url_text)
