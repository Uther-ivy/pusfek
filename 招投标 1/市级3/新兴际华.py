...
class alashan_ggzy:
    def __init__(self):
        ...
        self.domain_name='http://www.jihuacaigou.com'
    ...
    def parse(self):
        ...
        while True:
            page += 1
            if page ==1:
                url=f'http://www.jihuacaigou.com/gonggao/index.jhtml'
            else:
                url=f'http://www.jihuacaigou.com/gonggao/index_{page}.jhtml'
            print('*' * 20, page, '*' * 20)
            text = tool.requests_get(url, self.headers).replace('&#x2f;', '/')
            html = HTML(text)
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath("//div[@class='List2 Top5']//li")
            for li in detail:
