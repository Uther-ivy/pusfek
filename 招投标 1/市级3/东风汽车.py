...
class alashan_ggzy:
    def __init__(self):
        ...
        self.domain_name='http://jyzx.dfmbidding.com'
    ...
    def parse(self):
        ...
        while True:
            page += 1
            if page==1:
                url='http://jyzx.dfmbidding.com/zbxx/index.jhtml'
            else:
                url = f'http://jyzx.dfmbidding.com/zbxx/index_{page}.jhtml'
            print('*' * 20, page, '*' * 20)
            text = tool.requests_get(url, self.headers).replace('&#x2f;', '/')
            html = HTML(text)
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath("//div[@class='lb-link']/ul/li")
            for li in detail:
                title = li.xpath('./a/span/text()')[0].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                url = li.xpath('./a//@href')[0]
                date_Today = li.xpath("./a/span[@class='bidDate']/text()")[0].replace('\xa0', '').replace('\n', ''). \
