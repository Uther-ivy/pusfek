...
class wenshan_ggzy:
    def __init__(self):
        self.domain_name = 'http://hgcg.customs.gov.cn'
        self.url_list = [
    ...
    def parse(self):
        ...
        while True:
            page += 1
            url=f'http://hgcg.customs.gov.cn/purchase/portal/portalArticle/article/announcements'
            payload = {
                'columnId': "af8b307d33da4a69ad8224c806e38a80",
                'pageNum': page,
                'pageSize': 25,
                'searchKey': "",
                'total': 222}
            text = tool.requests_get(url , self.headers,param=payload)
            print('*' * 20, page, '*' * 20)
            ...
            for li in detail:
                title = li['title']
                cid= li['id']
                url = f'http://hgcg.customs.gov.cn/purchase/portal/portalArticle/article/{cid}'
    ...
    def parse_detile(self, title, url, date):
        ...
        #     int('a')
        item = {}
        item['title'] = data.get('title').replace('\u2022', '')
        item['url'] = url
        item['date'] = date
