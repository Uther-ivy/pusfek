...
class baoshan_ggzy:
    def __init__(self):
        self.url_list = [
          'https://ss.ebnew.com/tradingSearch/index.htm?infoClassCodes=&rangeType=&projectType=bid&fundSourceCodes=&dateType=&startDateCode=&endDateCode=&normIndustry=&normIndustryName=&zone=&zoneName=&zoneText=&key=&pubDateType=&pubDateBegin=&pubDateEnd=&sortMethod=timeDesc&orgName=&currentPage={}'
        ]
    ...
    def parse(self):
        ...
        while True:
            ...
            for li in detail:
                title = li.xpath('//div[@class="abstract-head bg-fff clearfix"]/a/@title')[0].replace('\n', '').replace('\t', '').replace(' ', '').replace('\r', '')
                url = li.xpath('//div[@class="abstract-head bg-fff clearfix"]/a/@href')[0]
                url_domain = 'https://ss.ebnew.com/'
                if 'http' not in url:
                    if '../../' in url:
                        ...
                        url = url_domain + url
                date_Today = li.xpath('//i[2]/text()')[0].replace('\n', '').replace('\t', '').replace(' ', '').replace(
                    '\r', '').replace('发布日期:','')
                if '测试' in title:
                    continue
