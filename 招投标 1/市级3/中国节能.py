...
class alashan_ggzy:
    def __init__(self):
        ...
        self.domain_name='https://www.ebidding.cecep.cn'
    ...
    def parse(self):
        ...
        while True:
            page += 1
            if page ==1:
                url=f'https://www.ebidding.cecep.cn/jyxx/001006/bidinfo.html'
            else:
                url=f'https://www.ebidding.cecep.cn/jyxx/001006/{page}.html'
            print('*' * 20, page, '*' * 20)
            text = tool.requests_get(url, self.headers).replace('&#x2f;', '/')
            html = HTML(text)
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath("//ul[@class='go-items']/li")
            for li in detail:
                title = li.xpath('./a//text()')[0].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                url = li.xpath('./a//@href')[0]
                date_Today = li.xpath(".//span[@class='go-time r']/text()")[0].replace('\xa0', '').replace('\n', ''). \
    ...
    def parse_detile(self, title, url, date):
        ...
        # print(url_text)
        detail = url_html.xpath("//div[@id='oldinfo']")[0]
