...
class alashan_ggzy:
    def __init__(self):
        ...
        self.domain_name='https://ebidding.aecc-mall.com'
    ...
    def parse(self):
        ...
        while True:
            page += 1
            url=f'https://ebidding.aecc-mall.com/cms/channel/ywgg1/index.htm?pageNo={page}'
            print('*' * 20, page, '*' * 20)
            text = tool.requests_get(url, self.headers).replace('&#x2f;', '/')
            html = HTML(text)
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath("//li[@class='huowuli']")
            for li in detail:
                title = li.xpath('./a/span/text()')[1].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                url = li.xpath('./a/@href')[0]
                date_Today = li.xpath("./a/span/em/text()")[0].replace('\xa0', '').replace('\n', ''). \
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath("//div[@class='zhengWentText']")[0]
        # print(url_text)
