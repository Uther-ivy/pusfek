...
class alashan_ggzy:
    def __init__(self):
        ...
        self.domain_name='http://www.ccgp-dalian.gov.cn'
    ...
    def parse(self):
        ...
        while True:
            ...
            if page==1:
                ...
                break# url = f'http://www.tobacco.gov.cn/gjyc/zfcg/list_{page}.shtml'
            print('*' * 20, page, '*' * 20)
            text = tool.requests_get_bm(url, self.headers).replace('&#x2f;', '/')
            html = HTML(text)
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath("//table[@id='MoreInfoList_DataGrid1']//tr")
            for li in detail:
                title = li.xpath('./td[2]/a/@title')[0].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                url = li.xpath('./td[2]/a/@href')[0]
                date_Today = li.xpath("./td[3]/text()")[0].replace('\xa0', '').replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                if 'http' not in url:
                    url = self.domain_name + url
                print(title, url, date_Today)
                # print(title.encode('gb18030'))
                # print(a2b_hex(title.encode('gb18030')))
                # time.sleep(666)
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath("//table[@id='_Sheet1']")[0]
        # print(url_text)
