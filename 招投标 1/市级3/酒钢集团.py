...
class alashan_ggzy:
    def __init__(self):
        ...
        self.domain_name='http://eps.jiugangbid.com:8000'
    ...
    def parse(self):
        ...
        while True:
            page+=1
            url='http://eps.jiugangbid.com:8000/f/tenderannquainqueryanns/tenderannquainqueryanns/NewannquainList?tenderMode=2&projectName=&displaytype=secondLevel'
            print('*' * 20, page, '*' * 20)
            data={
                'pageNo': page,
                'pageSize': 20,
            ...
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath("//dl[@class='byTradingDetailParent clear']")
            for li in detail:
                title = li.xpath('./dd/a/text()')[0].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                url = li.xpath('./dd/a/@href')[0]
                date_Today = li.xpath(".//span[@class='gsPropertyLeftTime']/text()")[0].replace('\xa0', '').replace('\n', ''). \
