...
class alashan_ggzy:
    def __init__(self):
        ...
        self.domain_name='http://www.chnenergybidding.com.cn'
    ...
    def parse(self):
        ...
        while True:
            page += 1
            url=f'http://www.chnenergybidding.com.cn/bidweb/001/001002/001002001/{page}.html'
            print('*' * 20, page, '*' * 20)
            text = tool.requests_get(url, self.headers).replace('&#x2f;', '/')
            html = HTML(text)
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath("//li[@class='right-item clearfix']")
            for li in detail:
                title = li.xpath('.//a[@class="infolink"]/text()')[0].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                url = li.xpath('.//a[@class="infolink"]/@href')[0]
                date_Today = li.xpath("./span[@class='r']/text()")[0].replace('\xa0', '').replace('\n', ''). \
