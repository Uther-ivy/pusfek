...
class wenshan_ggzy:
    def __init__(self):
        self.domain_name = 'http://jiuanjiaotu.com'
        self.url_list = [
    ...
    def parse(self):
        ...
        while True:
            page += 1
            url=f'http://jiuanjiaotu.com/bidcenter/moreHXsuresearch.do?order=asc&formVal=%7B%22title%22%3A%22%22%2C%22datepicker1%22%3A%22%22%2C%22datepicker2%22%3A%22%22%2C%22bidcenter_selecttype%22%3A%22zbgg%22%2C%22industry_select%22%3A%22%22%2C%22area_select%22%3A%22%22%7D&_={int(time.time()*1000)}'
            payload = {
                'noticeType':  1,
                'pageSize': 10,
                'pageNo': page,
            ...
            for li in detail:
                title = li['name']
                cid= li['id']
                url = f'http://jiuanjiaotu.com/jiaoyi/details.do?categoryid=zbgg&infoid={cid}&bgggid=' \

                city=li['area']
                date_Today = int(li['time']/1000)
                if 'http' not in url:
                    url = self.domain_name + url
                print(title, url, date_Today)
                # time.sleep(666)
                endtime=int(li['endtime']/1000)
    ...
    def parse_detile(self, title, url, date,city,cid,endtime):
        print(url)
        jsonurl=f'http://jiuanjiaotu.com/jiaoyi/queryBy.do?id={cid}&type=zbgg&bgggid='
        data={
            'id': cid,
            'isDetail': 1
        }
        ...
        #     except:
        #         item['endtime'] = int(time.mktime(time.strptime(str(date), "%Y-%m-%d")))
