...
class wenshan_ggzy:
    def __init__(self):
        self.domain_name = 'http://www.jxtb.org.cn/'
        self.url_list = []
        self.headers = {
    ...
    def parse(self):
        ...
        while True:
            page += 1
            url=f'http://www.jxtb.org.cn/gongshigg/zhaobiaogg/?p-{page}.html'
            ...
            detail = html.xpath("//div[@class='newlist']//li")
            for li in detail:
                title = li.xpath(f'./a/text()')[0].replace('\n', '').replace('\r', '').replace('\t', '').replace(' ', '')
                url = li.xpath(f'./a/@href')[0]
                date_Today = li.xpath(f"./span/text()")[0].replace('\xa0', '').replace('\n', ''). \
    ...
    def parse_detile(self, title, url, date):
        print(url)
        t = tool.requests_get_GBK(url, self.headers).replace('\xa9', '').replace('<br>&nbsp;&nbsp;', '') \
            .replace('</o:p><', '').replace('<o:p><', '')
        # print(t)
        # time.sleep(2222)
        url_html = etree.HTML(t)
        detail = url_html.xpath("//div[@class='v_news_content']")[0]
