...
class alashan_ggzy:
    def __init__(self):
        ...
        self.domain_name='http://www.zfcg.cee.edu.cn'
    ...
    def parse(self):
        ...
        while True:
            page += 1
            url=f'http://www.zfcg.cee.edu.cn/purchase/portal/purchasetList/1?pageNum={page}'
            print('*' * 20, page, '*' * 20)
            text = tool.requests_get(url, self.headers).replace('&#x2f;', '/')
            html = HTML(text)
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath("//div[@class='list pagelist font']/ul/li")
            for li in detail:
                title = li.xpath('./a/text()')[0].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                url = li.xpath('./a/@href')[0]
                date_Today = li.xpath("./span[@class='field-content stime']/text()")[0].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\xa5', '').replace('发布时间：','').replace('/', '-')
                if 'http' not in url:
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath("//div[@class='col-md-8 col-md-offset-2']")[0]
        # print(url_text)
