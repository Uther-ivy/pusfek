...
class alashan_ggzy:
    def __init__(self):
        ...
        self.domain_name='http://hbzjk.86ztb.com'
    ...
    def parse(self):
        ...
        while True:
            page+=1
            url='http://hbzjk.86ztb.com/finddata.htm'
            print('*' * 20, page, '*' * 20)
            data={
                'org.apache.struts.taglib.html.TOKEN': 'd2cc6002445d3d537c60de0446d22793',
                'htm': 'dataList',
                'postmode': '2',
                'id': '0',
                'ids': '',
            ...
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath("//tr[@class='middle']")
            for li in detail:
                title = li.xpath('./td[@class="autoline"]/a/text()')[0].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                urls = li.xpath('./td[@class="autoline"]/a/@onclick')[0]
                url=re.findall(r'window.open\(\'(.*)\'\)\;',urls)[0]
                date_Today = li.xpath("./td[@class='tc'][7]/text()")[0].replace('\xa0', '').replace('\n', ''). \
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath("//div[@id='ggmb']")[0]
