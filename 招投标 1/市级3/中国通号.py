...
class alashan_ggzy:
    def __init__(self):
        ...
        self.domain_name='http://thzb.crsc.cn'
    ...
    def parse(self):
        ...
        while True:
            page += 1
            url=f'http://thzb.crsc.cn/g2586/m5978/mp{page}.aspx'
            print('*' * 20, page, '*' * 20)
            text = tool.requests_get(url, self.headers).replace('&#x2f;', '/')
            html = HTML(text)
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath("//div[@class='second-lb-item']")
            for li in detail:
                title = li.xpath('.//a/text()')[0].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                url = li.xpath('.//a/@href')[0]
                date_Today = li.xpath("./div[@class='second-lb-item-date']//text()")[0].replace('\xa0', '').replace('\n', ''). \
