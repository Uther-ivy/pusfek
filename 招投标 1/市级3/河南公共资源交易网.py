...
class baoshan_ggzy:
    def __init__(self):
        self.url_list = [
          'http://www.hnggzy.net/EpointWebBuilder/rest/frontAppCustomAction/getPageInfoListNewYzm'
        ]
    ...
    def parse(self):
        ...
        page = 0
        while True:
            data=f"siteGuid=7eb5f7f1-9041-43ad-8e13-8fcb82ea831a&categoryNum=002&kw=&startDate=&endDate=&pageIndex=1&pageSize=8&jytype="
            text = tool.requests_post2(self.url, headers=self.headers,data=data)
            page += 1
            print('*' * 20, page, '*' * 20)
            print(text,data)
            # print(11, text)
            ...
            for li in detail:
                title = li['title']
                url = 'http://www.hnggzy.net/'+li['infourl']

                date_Today = li['startdate'].split(' ')[0]
                if '测试' in title:
                    continue
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//div[@class="main"]')[0]
