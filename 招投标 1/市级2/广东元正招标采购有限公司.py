...
class xinyang_ggzy:
    ...
    def parse(self):
        date = tool.date
        date_ = time.strftime('%Y-%m-%d', time.localtime(time.time()+86400))
        date_to = time.strftime('%Y-%m-%d', time.localtime(time.time()-86400*10))
        # date = '2020-07-03'
        page = 0
        while True:
            page += 1
            text = tool.requests_get(self.url.format((page-1)*20, date_to, date_, page), self.headers)
            # print(text)
            # html = HTML(text)
            print('*' * 20, page, '*' * 20)
            ...
            for li in detail:
                ...
                url = 'http://www.gdbidding.com/zbxxDetail?id=' + \
                      str(li['id'])
    ...
    def parse_detile(self, title, url, date, cont):
        print(url)
        detail_html = cont
        detail_text = ''.join(re.findall('>(.*?)<', cont, re.S)) \
            .replace('\xa0', '').replace('\n', '').replace('\r', '').replace('\t', '') \
