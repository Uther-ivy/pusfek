...
class wenshan_ggzy:
    def __init__(self):
        self.domain_name = 'http://www.hzzbb.com/'
        self.url_list = [
    ...
    def parse(self):
        ...
        while True:
            ...
            # time.sleep(6666)
            if 'zhaobiao' in self.url:
                url_list = re.findall('''<div align="left"><SPAN class=a01><A target="_blank" HREF="(.*?)">(.*?)</A></SPAN></div></TD>''', text)
            ...
            for li in range(len(url_list)):
                ...
                date_Today = date_list[li].replace('\xa0', '').replace('\n', '').\
                    ...
                    .replace('[', '').replace(']', '')
                d = date_Today.replace(date[:5], '')
                if int(d[0]) < 10 and d[1] == '-':
                    date_Today = date[:5] + '0' + d
                if 'http' not in url:
    ...
    def parse_detile(self, title, url, date):
        ...
        try:
            detail = url_html.xpath('/html/body/center/table[5]/tbody/tr/td[2]/table[6]/tbody/tr')[0]
            detail_html = etree.tostring(detail, method='HTML')
            detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
            detail_text = url_html.xpath('string(/html/body/center/table[5]/tbody/tr/td[2]/table[6]/tbody/tr)').replace('\xa0', '').replace('\n',
                                                                                                                 ''). \
                replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
            if len(detail_html) < 200:
                int('a')
        except:
            detail = url_html.xpath('//*[@id="Tr9"]')[0]
            detail_html = etree.tostring(detail, method='HTML')
            detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
            detail_text = url_html.xpath('string(//*[@id="Tr9"])').replace(
                '\xa0', '').replace('\n',
                                    ''). \
                replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
            if len(detail_html) < 200:
    ...
    def get_nativeplace(self, addr):
        city = ''
        city_list = [['14007.001', '汉台区'], ['14007.01', '留坝县'], ['14007.011', '佛坪县'], ['14007.002', '南郑县'], ['14007.003', '城固县'], ['14007.004', '洋县'], ['14007.005', '西乡县'], ['14007.006', '勉县'], ['14007.007', '宁强县'], ['14007.008', '略阳县'], ['14007.009', '镇巴县']]
