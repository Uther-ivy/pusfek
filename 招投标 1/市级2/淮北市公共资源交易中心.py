...
class wenshan_ggzy:
    def __init__(self):
        self.domain_name = 'http://ggzy.huaibei.gov.cn'
        self.url_list = [
            '002001',
            '002002'
        ]
    ...
    def parse(self):
        ...
        while True:
            data = {'params': '{"siteGuid":"7eb5f7f1-9041-43ad-8e13-8fcb82ea831a","categoryNum":"'+self.url+'","keyword":"","startDate":"","endDate":"","publishDate":"","area":"","tradeType":"","pageIndex":'+str(page)+',"pageSize":12}'}
            page += 1
    ...
    def parse_detile(self, title, url, date):
        ...
        try:
            detail = url_html.xpath('//*[@id="iframeList0"]')[0]
            detail_html = etree.tostring(detail, method='HTML')
            detail_html = html.unescape(detail_html.decode()).replace("display: none", '').replace("\u2022", '').replace('\xa0', '')
            detail_text = url_html.xpath('string(//*[@id="iframeList0"])').replace('\xa0', '').replace('\n',
                                                                                                                 ''). \
                replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
            if len(detail_html) < 200:
                int('a')
        except:
            c = re.findall('infoid=(.*?)&categorynum', url)[0]
            c_ = url.split('categorynum=')[1]
            url_to = 'http://ggzy.huaibei.gov.cn/jyxx/{}/{}/{}/{}/{}.html'.format(c_[:6],c_[:9],c_,date.replace('-', ''),c)
            t = tool.requests_get(url_to, self.headers).replace('\xa9', '').replace('<br>&nbsp;&nbsp;', '')
            url_html = etree.HTML(t)
    ...
    def get_nativeplace(self, addr):
        city = ''
        city_list = [['6506.001', '杜集区'], ['6506.002', '相山区'], ['6506.003', '烈山区'], ['6506.004', '濉溪县']]
