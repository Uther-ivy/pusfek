...
def get_info(infourl,title,data):
    print(infourl)
    resp = requests.get(infourl, headers=headers,proxies=pro,timeout=20)
    resp.encoding = "utf-8"
    sel = Selector(resp)
    img = sel.xpath('//img[@border="0"]/@src').extract_first()
    item = {}
    item['zhao_time'] = data
    item['title'] = title
    ...
    if img == None:
        try:
            item['body'] = sel.xpath('//div[@style="display:none"]').extract_first().replace("display:none","")
        except:
            return
        ...
        item['function'] = tool.get_function(item['detail'])
    else:
        item['body'] = '<img border="0" src="http://www.ggzyjypt.com.cn' + img + "\">"
        item['detail'] = item['body'].replace('\n', '').replace("\t", '').replace("\r", '').replace('\xa0', '').replace('\xc2','').replace(' ','')
        item['endtime'] = tool.get_endtime(item['detail'])
        if item['endtime'] == '':
            item['endtime'] = int(time.mktime(time.strptime(data, "%Y-%m-%d")))
    ...
    for catenum in categorynums:
        ...
        for da in date:
            ...
            if tool.Transformation(data) <= tool.Transformation(now_time):
                infourl = "http://www.ggzyjypt.com.cn" + info_url + "?diqu=620901"
                get_info(infourl,title,data)
            else:
                print(data)
