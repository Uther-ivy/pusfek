...
class baoshan_ggzy:
    ...
    def parse(self):
        while True:
            ...
            html = HTML(text).xpath('//*[@class="list-ul dh"]/li')
            for li in html:
                url= 'http://www.lncc.edu.cn/' + li.xpath('./a/@href')[0]
                title = li.xpath('./a/@title')[0].strip()
                date_Today = li.xpath('./span[2]/text()')[0].replace('年','-').replace('月','-').replace('日','').strip()
                try:
                    if '招标' not in li.xpath('./span[1]/text()')[0]:
                        print('不是招标公告')
                        continue
                except:
                    print('不是招标公告')
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//div[@id="vsb_content"]')[0]
        # print(detail)
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode())
        detail_text = url_html.xpath('string(//div[@id="vsb_content"])').replace('\xa0', '').replace('\n', ''). \
