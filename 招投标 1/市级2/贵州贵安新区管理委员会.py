...
class alashan_ggzy:
    def __init__(self):
        self.url_code = [
            'http://www.gaxq.gov.cn/zwgk/xxgkml/zdlyxxgk/ggzypz/zfcg2/{}.html',
        ]
    ...
    def parse(self):
        ...
        while True:
            ...
            print('*' * 20, page, '*' * 20)
            if page == 1:
                text = tool.requests_get(self.url.format('xxgk_list'), self.headers)
            else:
                text = tool.requests_get(self.url.format('xxgk_list_' + str(page-1)), self.headers)

            detail = etree.HTML(text).xpath('//*[@id="idData"]/tr')
            for li in detail:
