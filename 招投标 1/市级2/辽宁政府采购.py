...
class liaoning_zfcg:
    def __init__(self):
        ...
        self.url = 'http://ccgp-liaoning.gov.cn/portalindex.do?method=getPubInfoList&t_k=null'
        self.headers = {
    ...
    def parse(self):
        ...
        while True:
            page += 1
            data['current'] = page
            data['infoTypeCode'] = self.code
            detail = json.loads(tool.requests_post(self.url, data, self.headers))['rows']
            print('*' * 20, page, '*' * 20)
            # print(11, detail)
            # time.sleep(6666)
            for li in detail:
                title = li['title']
                url = 'http://ccgp-liaoning.gov.cn/portalindex.do?method=getPubInfoViewOpenNew&infoId='+li['id']
    ...
    def parse_detile(self, title, url, date):
        print(url)
        text = tool.requests_get(url, self.headers).replace('&lt;', '<').replace('&gt;', '>').replace('&#034;', '"')
        # print(text)
        # time.sleep(6666)
        url_html = etree.HTML(text)
        try:
            detail = url_html.xpath('//*[@id="template"]/form/table/tbody/tr/td/table/tbody/tr[2]/td[2]/table/tbody/tr/td/table')[0]
        except:
            return
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode())
        detail_text = url_html.xpath('string(//*[@id="template"]/form/table/tbody/tr/td/table/tbody/tr[2]/td[2]/table/tbody/tr/td/table)').replace('\xa0', '').replace('\n', '').\
