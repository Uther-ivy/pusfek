...
class alashan_ggzy:
    def __init__(self):
        self.url_code = [
            'https://www.baohuabidding.com/EpointWebBuilder/rest/GgSearchAction/getInfoMationList',
        ]
    ...
    def parse(self):
        ...
        page = 0
        while True:
            data = {'params': '{"siteGuid":"7eb5f7f1-9041-43ad-8e13-8fcb82ea831a","zbjg":"","categoryNum":"001","keyword":"","pageIndex":'+str(page)+',"pageSize":10}'}
            page += 1
            print('*' * 20, page, '*' * 20)
            ...
            for li in detail:
                title = '[' + li['addressname'] + ']' + li['title']
                url = 'https://www.baohuabidding.com' + li['infourl']
                date_Today = li['infodate'].replace('\r', '').replace('\t', '').replace(' ', '')
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('/html/body/div[2]/div/div[2]/div/div[4]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode())
        detail_text = url_html.xpath('string(/html/body/div[2]/div/div[2]/div/div[4])').replace('\xa0', '').replace('\n',
                                                                                                           '').replace(
            '\r', '').replace('\t',
                              '').replace(
            ' ', '').replace('\xa5', '')
