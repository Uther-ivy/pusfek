...
def get_zhaobiao_info(zhao_time,info_url):
    ...
    item['body'] = sel.xpath('//div[@class="nes_details"]').extract_first()
    item['detail'] = item['body'].replace('\n', '').replace("\t", '').replace("\r", '').replace('\xa0', '').replace('\xc2', '').replace(' ', '')
    item['senddate'] = int(time.time())
    item['mid'] = 1403
    item['title'] = sel.xpath('//div[@class="nes_details"]/h2/text()').extract_first()
...
def get_project(url):
    ...
    resp.encoding="utf8"
    sel = Selector(resp)
    titles = sel.xpath('//table[@class="common_table"]/tbody/tr')
    now_time = tool.date
    # now_time = '2021-07-26'
    for ti in titles:
        # title = ti.xpath('./td/a/text()').extract_first()
        zhao_time = ti.xpath('./td[last()]/text()').extract_first().split(" ")[0]
        info_url = "http://emall.hust.edu.cn/" + ti.xpath('./td/a/@href').extract_first()
        if tool.Transformation(now_time) <= tool.Transformation(zhao_time):
            get_zhaobiao_info(zhao_time,info_url)
        else:
...
if __name__ == '__main__':
    ...
    try:
        ...
        for i in range(1, 4):
            url = "http://emall.hust.edu.cn/results.html?page={}"
