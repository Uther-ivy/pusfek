...
class wenshan_ggzy:
    def __init__(self):
        self.domain_name = 'http://www.guangxibid.com.cn'
        self.url_list = [
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('/html/body/div[3]/div[2]/div[2]/div/div[2]/div[2]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace("display: none", '').replace("\u2022", '').replace('\xa0', '')
        detail_text = url_html.xpath('string(/html/body/div[3]/div[2]/div[2]/div/div[2]/div[2])').replace('\xa0', '').replace('\n',
                                                                                                             ''). \
            replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
