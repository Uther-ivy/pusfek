...
class liaoning_ggzy:
    def __init__(self):
        self.url_list = [
            # 建设工程
            'http://www.lnggzy.gov.cn/lnggzy/showinfo/Morejyxx.aspx?timebegin={}&timeend={}&timetype=04&num1=002&num2=002000&jyly=005&word=',
            # 政府采购
            'http://www.lnggzy.gov.cn/lnggzy/showinfo/Morejyxx.aspx?timebegin={}&timeend={}&timetype=04&num1=001&num2=001000&jyly=005&word='
        ]
    ...
    def parse(self):
        date = tool.date
        date_str = time.strftime('%Y-%m-%d', time.localtime(time.time()-86400*10))
        # date = '2020-02-12'
        page = 0
        data = {
            '__VIEWSTATE': '',
        ...
        while True:
            ...
            print('*' * 20, page, '*' * 20)
            if page == 1:
                text = tool.requests_get(self.url.format(date_str, date), self.headers)
            else:
                text = tool.requests_post(self.url.format(date_str, date), data, self.headers)
            html = HTML(text)
            ...
            for li in detail:
                title = li.xpath('./td/div/div/h4/a/@title')[0].strip()
                url = 'http://www.lnggzy.gov.cn' + li.xpath('./td/div/div/h4/a/@href')[0]
    ...
    def parse_detile(self, title, url, date):
        if '005002002001' in url:
            url = 'http://www.lnggzy.gov.cn/lnggzy/ZtbInfo/Jsgc.aspx?'+ url.replace('http://www.lnggzy.gov.cn/lnggzy/InfoDetail/Default.aspx?', '')
        else:
            url = 'http://www.lnggzy.gov.cn/lnggzy/ZtbInfo/Zfcg.aspx?' + url.replace(
                'http://www.lnggzy.gov.cn/lnggzy/InfoDetail/Default.aspx?', '')
        print(url)
        ...
        try:
            detail = url_html.xpath('//*[@id="jsgc_zbgg1_spnShow"]/table')[0]
            detail_html = etree.tostring(detail, method='HTML')
            detail_html = html.unescape(detail_html.decode())
            detail_text = url_html.xpath('string(//*[@id="jsgc_zbgg1_spnShow"]/table)').replace('\xa0', '').replace('\n', '').\
                replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
        except:
            try:
                detail = url_html.xpath('//*[@id="jsgc_zbgg1_tblInfo"]')[0]
                detail_html = etree.tostring(detail, method='HTML')
                detail_html = html.unescape(detail_html.decode())
                detail_text = url_html.xpath('string(//*[@id="jsgc_zbgg1_tblInfo"])').replace('\xa0', '').replace(
                    '\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
            except:
                try:
                    detail = url_html.xpath('//*[@id="jsgc_zbgg1_TDContent"]/div/table')[0]
                    detail_html = etree.tostring(detail, method='HTML')
                    detail_html = html.unescape(detail_html.decode())
                    detail_text = url_html.xpath('string(//*[@id="jsgc_zbgg1_TDContent"]/div/table)').replace('\xa0', '').replace(
                        '\n', ''). \
                        replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                except:
                    try:
                        detail = url_html.xpath('//*[@id="zfcg_zbgg1_TDContent"]')[0]
                        detail_html = etree.tostring(detail, method='HTML')
                        detail_html = html.unescape(detail_html.decode())
                        detail_text = url_html.xpath('string(//*[@id="zfcg_zbgg1_TDContent"])').replace('\xa0',
                                                                                                        '').replace(
                            '\n', ''). \
                            replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                    except:
                        try:
                            ...
                            detail_html = etree.tostring(detail, method='HTML')
                            detail_html = html.unescape(detail_html.decode())
                            detail_text = url_html.xpath('string(//*[@id="_Sheet1"])').replace('\xa0',
                                                                                                            '').replace(
                                '\n', ''). \
                                replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                        except:
                            try:
                                detail = url_html.xpath('//*[@id="zfcg_zbgs1_TDContent"]')[0]
                                detail_html = etree.tostring(detail, method='HTML')
                                detail_html = html.unescape(detail_html.decode())
                                detail_text = url_html.xpath('string(//*[@id="zfcg_zbgs1_TDContent"])').replace('\xa0',
                                                                                                   '').replace(
                                    '\n', ''). \
                                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                            except:
                                try:
                                    detail = url_html.xpath('//*[@id="zfcg_htgg1_TDContent"]')[0]
                                    detail_html = etree.tostring(detail, method='HTML')
                                    detail_html = html.unescape(detail_html.decode())
                                    detail_text = url_html.xpath('string(//*[@id="zfcg_htgg1_TDContent"])').replace('\xa0',
                                                                                                                    '').replace(
                                        '\n', ''). \
                                        replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                                except:
                                    detail = url_html.xpath('//*[@id="zfcg_bggg1_TDContent"]')[0]
                                    detail_html = etree.tostring(detail, method='HTML')
                                    detail_html = html.unescape(detail_html.decode())
                                    detail_text = url_html.xpath('string(//*[@id="zfcg_bggg1_TDContent"])').replace(
                                        '\xa0',
                                        '').replace(
                                        '\n', ''). \
                                        replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
