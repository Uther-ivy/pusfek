...
def get_zhaobiaoinfo(info_url,title):
    print(info_url)
    ...
    item['body'] = sel.xpath('//*[@id="2020_VERSION"]').extract_first()
    item['detail'] = item['body'].replace('\n', '').replace("\t", '').replace("\r", '').replace('\xa0', '').replace('\xc2', '').replace(' ', '')
    item['zhao_time'] = re.findall('发布时间：[\S\s]*?</span>',item['body'])[0].replace('\n','').replace(' ','').split('发布时间：<span>')[1].split('</span>')[0]
    item['senddate'] = int(time.time())
    item['mid'] = 1403
    item['resource'] = '河北省政府采购网'
...
def get_project(url):
    ...
    resp.encoding='utf8'
    sel = Selector(resp)
    titles = len(sel.xpath('//*[@id="moredingannctable"]/tr'))
    for ti in range(1, titles+1, 2):
        title = sel.xpath('//*[@id="moredingannctable"]/tr[{}]/td[2]/a/text()'.format(ti)).extract_first()
        info_url = url.split('index')[0] + sel.xpath('//*[@id="moredingannctable"]/tr[{}]/td[2]/a/@href'.format(ti)).extract_first().replace('./','')
        date_today = sel.xpath('//*[@id="moredingannctable"]/tr[{}]/td[2]/span[1]/text()'.format(ti+1)).extract_first()
        # print(title,info_url, date_today)
        # time.sleep(2222)
        if tool.Transformation(date) <= tool.Transformation(date_Today):
            get_zhaobiaoinfo(info_url,title)
        else:
