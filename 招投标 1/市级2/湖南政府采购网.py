...
class xinyang_ggzy:
    def __init__(self):
        self.url_list = ['http://www.ccgp-hunan.gov.cn/mvc/getNoticeList4Web.do']
        self.url = self.url_list.pop(0)
        self.headers = {
            'Accept': 'application/json, text/javascript, */*; q=0.01',
            'Host': 'www.ccgp-hunan.gov.cn',
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.100 Safari/537.36',
            # 'Cookie': 'JSESSIONID=n3gchJLCX1tpG47hv67G4G2NnHNdp2snG0lTPJZcxTvnJzB2xj6y!-1541843936',
            'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
        }

    def parse(self):
        date = tool.date
        date_to = time.strftime('%Y-%m-%d', time.localtime(time.time()-86400*1))
        # date = '2020-07-10'
        page = 0
        data = 'pType=&prcmPrjName=&prcmItemCode=&prcmOrgName=&startDate={}&endDate={}&prcmPlanNo=&page={}&pageSize=18'
        while True:
            page += 1
            text = tool.requests_post(self.url, data.format(date_to, date, page), self.headers)
            # print(text)
            # time.sleep(6666)
            ...
            for tr in detail:
                ...
                url = 'http://www.ccgp-hunan.gov.cn/mvc/viewNoticeContent.do?noticeId={}&area_id='.format(tr['NOTICE_ID'])
