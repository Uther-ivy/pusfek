...
class baoshan_ggzy:
    def __init__(self):
        self.url_list = [
            'http://zbtb.gxi.gov.cn:9000/xxfbcms/category/changeBulletinList.html?searchDate=1996-07-30&dates=300&word=&categoryId=89&industryName=&area=&status=&publishMedia=&sourceInfo=&showStatus=&page={}',
        ]
        ...
        while True:
            ...
            for li in detail:
                try:
                    ...
                    continue
                title = li.xpath('./td[1]/a/@title')[0].strip()
                date_Today = li.xpath('./td[4]/text()')[0].replace("\r", '').replace("\n", '').replace("\t", '').replace(" ", '')[:10]
                if '发布' in date_Today:
                    continue
                if '测试' in title:
                    continue
    ...
    def parse_detile(self, title, url, date):
        ...
        detail_html = f'''<embed style="width: 100%; height: 1060px; display: block" src='{u}'> </embed>'''
        detail_text = ''
        item = {}
