...
class wenshan_ggzy:
    def __init__(self):
        self.domain_name = 'http://zbcg.mas.gov.cn'
        self.url_list = [
    ...
    def parse(self):
        ...
        while True:
            page += 1
            if page == 1:
                text = tool.requests_get(self.url.format('tradelist'), self.headers)
            else:
    ...
    def parse_detile(self, title, url, date):
        ...
        try:
            detail = url_html.xpath('//*[@id="jiazai0"]/div/div[2]/div[2]')[0]
            detail_html = etree.tostring(detail, method='HTML')
            detail_html = html.unescape(detail_html.decode()).replace("display: none", '').replace("\u2022", '').replace('\xa0', '')
            detail_text = url_html.xpath('string(//*[@id="jiazai0"]/div/div[2]/div[2])').replace('\xa0', '').replace('\n',
                                                                                                                 ''). \
                replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
            ...
            try:
                ...
                detail = url_html.xpath('/html/body/div[2]/div/div[2]/div[1]/div[3]/div/div[1]')[0]
                detail_html = etree.tostring(detail, method='HTML')
                detail_html = html.unescape(detail_html.decode()).replace("display: none", '').replace("\u2022",
                                                                                                       '').replace(
                    '\xa0',
                    '')
                detail_text = url_html.xpath('string(/html/body/div[2]/div/div[2]/div[1]/div[3]/div/div[1])').replace('\xa0', '').replace(
                    '\n',
                    ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                if len(detail_html) < 200:
