...
class wenshan_ggzy:
    def __init__(self):
        self.domain_name = 'http://14.215.227.219:81'
        self.url_list = [
             'http://14.215.227.219:81/jsgc/{}.html',#结果
             'http://14.215.227.219:81/zfcg/{}.html'
        ]
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//*[@class="xiangxiyekuang"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
        detail_text = url_html.xpath('string(//*[@class="xiangxiyekuang"])').replace('\xa0', '').replace('\n',
                                                                                                             ''). \
            replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
    ...
    def get_nativeplace(self, addr):
        city = ''
        city_list = [['10014.001', '源城区'], ['10014.002', '紫金县'], ['10014.003', '龙川县'], ['10014.004', '连平县'], ['10014.005', '和平县'], ['10014.006', '东源县']]
