...
class alashan_ggzy:
    def __init__(self):
        self.url_code = [
            'http://www.crpsz.com/zbxx/006001/{}.html',
            'http://www.crpsz.com/zbxx/006002/{}.html'
        ]
    ...
    def parse(self):
        ...
        while True:
            ...
            print('*' * 20, page, '*' * 20)
            if page == 1:
                text = tool.requests_get(self.url.format('secondpagejy'), self.headers)
            else:
                text = tool.requests_get(self.url.format(page), self.headers)
            # print(111, text)
            # time.sleep(666)
            detail = etree.HTML(text).xpath('//*[@id="infocontent"]/tr')
            for li in detail:
                title = li.xpath('./td[2]/a/text()')[0]
                url = 'http://www.crpsz.com' + li.xpath('./td[2]/a/@href')[0]
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//*[@id="lw_ft"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode())
        detail_text = url_html.xpath('string(//*[@id="lw_ft"])').replace('\xa0', '').replace('\n',
                                                                                                           '').replace(
            '\r', '').replace('\t',
                              '').replace(
            ' ', '').replace('\xa5', '')
