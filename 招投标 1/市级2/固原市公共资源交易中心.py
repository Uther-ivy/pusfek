...
class wenshan_ggzy:
    def __init__(self):
        self.domain_name = 'http://www.gysggzyjy.cn'
        self.url_list = [
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//*[@id="acticlesm"]/div[1]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace("display: none", '').replace("\u2022", '').replace('\xa0', '')
        detail_text = url_html.xpath('string(//*[@id="acticlesm"]/div[1])').replace('\xa0', '').replace('\n',
                                                                                                             ''). \
            replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
    ...
    def get_nativeplace(self, addr):
        city = ''
        city_list = [['15504.001', '原州区'], ['15504.002', '西吉县'], ['15504.003', '隆德县'], ['15504.004', '泾源县'], ['15504.005', '彭阳县']]
