...
class baoshan_ggzy:
    def __init__(self):
        self.url_list = [
            'http://sthj.ln.gov.cn/xxgk/czzj/zbcg/',
        ]
    ...
    def parse(self):
        while True:
            ...
            html = HTML(text)
            detail = html.xpath('//div[@class="zy_list2"]//li')
            for li in detail:
                url= 'http://sthj.ln.gov.cn/xxgk/czzj/zbcg/'+ li.xpath('./a/@href')[0].replace('./', '')
                title = li.xpath('./a/@title')[0]
                date_Today = li.xpath('./div/text()')[0] +'-'+ li.xpath('./div/span/text()')[0]
                if '发布' in date_Today:
                    continue
                if '测试' in title:
                    continue
