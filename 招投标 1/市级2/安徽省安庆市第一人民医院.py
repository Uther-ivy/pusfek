...
class alashan_ggzy:
    def __init__(self):
        self.url_code = [
            'http://www.aqhospital.com/list.php?pid=3&ty=75&page={}'
        ]
    ...
    def parse(self):
        ...
        while True:
            ...
            detail = etree.HTML(text).xpath('/html/body/div[4]/div[2]/div[2]/ul/li')
            # print(111, detail)
            # time.sleep(666)
            for li in detail:
                title = li.xpath('./a/text()')[0]
                url = 'http://www.aqhospital.com/' + li.xpath('./a/@href')[0]
                date_Today = li.xpath('./span/text()')[0].replace('[', '').replace(']', '')
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('/html/body/div[4]/div[2]/div[2]/div/div[3]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode())
        detail_text = url_html.xpath('string(/html/body/div[4]/div[2]/div[2]/div/div[3])').replace('\xa0', '').replace('\n', '').replace('\r', '').replace('\t',
                                                                                                                  '').replace(
            ' ', '').replace('\xa5', '')
        item = {}
    ...
    def get_nativeplace(self, addr):
        city_list = [['6508.001', '迎江区'], ['6508.01', '岳西县'], ['6508.011', '桐城市'], ['6508.002', '大观区'], ['6508.003', '郊区'], ['6508.004', '怀宁县'], ['6508.005', '枞阳县'], ['6508.006', '潜山县'], ['6508.007', '太湖县'], ['6508.008', '宿松县'], ['6508.009', '望江县']]
