...
class baoshan_ggzy:
    def __init__(self):
        self.url_list = [
            'http://jt.hlj.gov.cn/fw/ztbxx/zbxx/',
            'http://jt.hlj.gov.cn/fw/ztbxx/zbgs/',
    ...
    def parse(self):
        while True:
            ...
            html = HTML(text)
            detail = html.xpath('//div[@class="list_content"]//li')
            for li in range(len(detail)):
                url='http://www.hljjt.gov.cn'+html.xpath(f'(//div[@class="list_content"]//li//a//@href)[{li+1}]')[0]
                title = ''.join(html.xpath(f'(//div[@class="list_content"]//li//a//text())[{li+1}]')).strip()
                date_Today  = ''.join(html.xpath(f'(//div[@class="list_content"]//li//span//text())[{li+1}]')).strip()
                if '发布' in date_Today:
                    continue
                if '测试' in title:
                    continue
    ...
    def parse_detile(self, title, url, date):
        t = tool.requests_get(url, self.headers).replace('/attachment/','http://jt.hlj.gov.cn/attachment/')
        url_html = etree.HTML(t)
        detail = url_html.xpath('//div[@class="third_attachment"]')[0]
        # print(detail)
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode())
        detail_text = url_html.xpath('string(//div[@class="third_attachment"])').replace('\xa0', '').replace('\n', ''). \
