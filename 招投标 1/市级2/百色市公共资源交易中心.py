...
class wenshan_ggzy:
    def __init__(self):
        self.domain_name = 'http://www.bsggzy.org.cn'
        self.url_list = [
            'http://www.bsggzy.org.cn/bsggzyjy/jyxx/{}.html',
        ]
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//*[@class="paragrph-box"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace("display: none", '').replace("\u2022", '').replace('\xa0', '')
        detail_text = url_html.xpath('string(//*[@class="paragrph-box"])').replace('\xa0', '').replace('\n',
                                                                                                             ''). \
            replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
    ...
    def get_nativeplace(self, addr):
        city = ''
        city_list = [['10510.001', '右江区'], ['10510.01', '田林县'], ['10510.011', '西林县'], ['10510.012', '隆林各族自治县'], ['10510.002', '田阳县'], ['10510.003', '田东县'], ['10510.004', '平果县'], ['10510.005', '德保县'], ['10510.006', '靖西县'], ['10510.007', '那坡县'], ['10510.008', '凌云县'], ['10510.009', '乐业县']]
