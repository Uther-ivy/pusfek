...
class wenshan_ggzy:
    def __init__(self):
        self.domain_name = 'http://ggzy.deqing.gov.cn'
        self.url_list = [
    ...
    def parse(self):
        ...
        while True:
            ...
            for li in detail:
                title = li.xpath('string(./p[1]/a)').replace('\xa0', '').replace('\n', ''). \
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//*[@class="body"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace("display: none", '').replace("\u2022", '').replace('\xa0', '')
        detail_text = url_html.xpath('string(//*[@class="body"])').replace('\xa0', '').replace('\n',
                                                                                                             ''). \
            replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
        # print(detail_html)
        # time.sleep(2222)
