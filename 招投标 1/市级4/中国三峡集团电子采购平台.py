...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'https://eps.ctg.com.cn/cms/channel/1ywgg1/index.htm?pageNo={}', #招标公告
            'https://eps.ctg.com.cn/cms/channel/1ywgg4/index.htm?pageNo={}'  #中标结果
                    ]
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//div[@class="main-text"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
        detail_text = url_html.xpath('string(//div[@class="main-text"])').replace('\xa0', '').replace('\n', ''). \
