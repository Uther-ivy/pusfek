...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'http://www.gyzbdl.cn/Bidding/zhaobiaogonggao/list_10_{}.html', #招标公告
                    ]
    ...
    def parse(self):
        ...
        while True:
            ...
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath('//div[@class="MoreCaseBoxCon"]/li')
            for li in detail:
                title = li.xpath('./h3/a/@title')[0]
                url = li.xpath('./h3/a/@href')[0]
                date_Today = li.xpath('./p[2]/text()')[0].replace('时间： ', '')
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//div[@id="preview"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
        detail_text = url_html.xpath('string(//div[@id="preview"])').replace('\xa0', '').replace('\n', ''). \
