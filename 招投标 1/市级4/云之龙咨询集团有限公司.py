...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'http://www.gxyunlong.cn/gonggao/index/mtype/2/p/{}.html', #招标公告
                    ]
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//div[@class="article__gg"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
        detail_text = url_html.xpath('string(//div[@class="article__gg"])').replace('\xa0', '').replace('\n', ''). \
