...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'http://www.snsggzy.com/JyWeb/JYXX/List_JSGC?subtype=210&subtype2=210010&searchText=&pageIndex={}&pageSize=15', #工程招标
           ]
    ...
    def parse(self):
        ...
        while True:
            ...
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath('//div[@class="right_new1"]/ul/li')
            for li in detail:
                title = li.xpath('./a/@title')[0]
                href = li.xpath('./a/@href')[0]
                date_Today = li.xpath('./a/span[4]/text()|./a/span[3]/text()')[0]
                url = 'http://www.snsggzy.com' + href
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//div[@class="news_nr"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
        detail_text = url_html.xpath('string(//div[@class="news_nr"])').replace('\xa0', '').replace('\n', '').\
