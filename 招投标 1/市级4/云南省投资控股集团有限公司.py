...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'https://www.cnyig.com/newslist/zbgg/{}/15.htm', #工程招标
           ]
    ...
    def parse(self):
        ...
        while True:
            ...
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath('//dl[@class="textList"]/dd')
            for li in detail[1:]:
                title = li.xpath('./div[2]/a/@title')[0]
                href = li.xpath('./div[2]/a/@href')[0]
                year = li.xpath('./div[1]/em/text()')[0]
                day = li.xpath('./div[1]/span/text()')[0]
                date_Today = year + '-' + day
                url = 'https://www.cnyig.com' + href
    ...
    def parse_detile(self, title, url, date):
        ...
        pdf = url_html.xpath('/html/body/div[5]/div[2]/div[6]/iframe/@src')
        item = {}
        if pdf:
            pdf = pdf[0]
            item['body'] = pdf
        else:
            detail = url_html.xpath('//div[@class="articleBox"]')[0]
            detail_html = etree.tostring(detail, method='HTML')
            detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
            detail_text = url_html.xpath('string(//div[@class="articleBox"]//text())').replace('\xa0', '').replace('\n', '').\
