...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'https://www.whzbtb.com/V2PRTS/TendererNoticeInfoList.do', #工程招标
           ]
    ...
    def parse(self):
        ...
        while True:
            ...
            detail = html["rows"]
            for li in detail:
                title = li["tenderPrjName"]
                date_Today = li["noticeStartDate"].split(' ')[0]
                id = li["id"]
                url = f'https://www.whzbtb.com/V2PRTS/TendererNoticeInfoDetail.do?%69%64={id}'
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//div[@class="pageRight_box"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
        detail_text = url_html.xpath('string(//div[@class="pageRight_box"])').replace('\xa0', '').replace('\n', '').\
        ...
        item['resource'] = '武汉市公共资源交易平台招标投标交易系统'
        item['shi'] = 6507
        item['sheng'] = 6500
        item['removal']= title
        # print(item["body"])
