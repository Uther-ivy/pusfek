...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'https://www.tjjngc.com.cn/es/ggList', #工程招标
           ]
    ...
    def parse(self):
        ...
        while True:
            ...
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath('//div[@class="tbody"]/table/tbody/tr')
            for li in detail:
                title = li.xpath('./td[3]/span/text()')[0]
                date_Today = li.xpath('./td[4]/span/text()')[0].strip(" ")
                onclick = li.xpath('./@onclick')[0].replace("urlChangeGG('1',", "").replace(")", "").replace("'",
                                                                                                             "").split(
                    ',')
                gcGuid = onclick[2]
                ggGuid = onclick[0]
                url = f'https://www.tjjngc.com.cn/cgxx/ggDetail?gcGuid={gcGuid}&ggGuid={ggGuid}'
