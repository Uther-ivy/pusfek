...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'http://www.gzggzy.cn/jyywjsgcfwjzzbgg/index_{}.jhtml', #招标公告
                    ]
    ...
    def parse(self):
        ...
        while True:
            ...
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath('//table[@class="table public-table procureBulletin-table"]/tbody/tr')
            for li in detail:
                title = ''.join(li.xpath('./td[2]//text()')).replace('\t', '') \
                    .replace(' ', '').replace('\xa5', '').replace('\ufeff', '').replace('\n', '')
                date_Today = li.xpath('./td[3]/span/text()')[0]
                href = li.xpath('./td[2]/a/@href')[0]
                url = 'http://www.gzggzy.cn' + href
    ...
    def parse_detile(self, title, url, date):
        ...
        detail_text = url_html.xpath('string(//div[@class="detail"])').replace('\xa0', '').replace('\n', '').\
