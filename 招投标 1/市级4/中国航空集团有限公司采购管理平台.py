...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'https://pur.airchina.com.cn/001/001001/{}.html'
                    ]
        self.url1='https://pur.airchina.com.cn/001/001001/moreinfo.html'
    ...
    def parse(self):
        ...
        while True:
            page += 1
            if page==1:
                text=tool.requests_get(url=self.url1, headers=self.headers)
            else:
            ...
            for li in detail:
                ...
                date_Today = li.xpath('td[3]/span/text()')[0]
                url = 'https://pur.airchina.com.cn' + href
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//div[@class="ewb-notice-copy"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
        detail_text = url_html.xpath('string(//div[@class="ewb-notice-copy"])').replace('\xa0', '').replace('\n', ''). \
        ...
        item['resource'] = '中国航空集团有限公司采购管理平台'
        item['shi'] = 6507
        item['sheng'] = 6500
        item['removal'] = title
        # print(item)
