...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'https://www.mengniu.com.cn/news/zbxx/p/{}.html', #招标公告
                    ]
    ...
    def parse(self):
        ...
        while True:
            ...
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath('//ul[@class="news_title_list"]/li')
            for li in detail:
                title = li.xpath('./a/text()')[0]
                date_Today = li.xpath('./span/text()')[0]
                href = li.xpath('./a/@href')[0]
                url= 'https://www.mengniu.com.cn' + href
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//div[@class="left_col"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
        detail_text = url_html.xpath('string(//div[@class="left_col"])').replace('\xa0', '').replace('\n', '').\
