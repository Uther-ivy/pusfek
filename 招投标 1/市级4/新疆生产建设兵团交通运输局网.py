...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'http://jtj.xjbt.gov.cn/xxgk_zcjd_zbzb/xxgk_zcjd_zbzb_zbgs/index_{}.shtml', #招标公告
                    ]
        self.url1='http://jtj.xjbt.gov.cn/xxgk_zcjd_zbzb/xxgk_zcjd_zbzb_zbgs/index.shtml'
    ...
    def parse(self):
        ...
        while True:
            page += 1
            if page==1:
                text = tool.requests_get(url=self.url1.format(page), headers=self.headers)
            else:
            ...
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath('//div[@class="list"]/div')
            for li in detail:
                title = li.xpath('./div[1]/a/text()')[0]
                date_Today = li.xpath('./div[2]/text()')[0]
                href = li.xpath('./div[1]/a/@href')[0]
                url = 'http://jtj.xjbt.gov.cn' + href
