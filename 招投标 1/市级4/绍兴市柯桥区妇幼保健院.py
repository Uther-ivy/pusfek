...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'https://www.kqqfeyy.com/news.php?id=10015&page={}', #招标公告
                    ]
    ...
    def parse(self):
        ...
        while True:
            ...
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath('//div[@class="news_list"]/ul/li')
            for li in detail:
                title = li.xpath('./div[2]/h2/a/text()')[0]
                date_Today = li.xpath('./div[2]/span/text()')[0].replace('发布时间：', '')
                href = li.xpath('./div[2]/h2/a/@href')[0]
                url = 'https://www.kqqfeyy.com/' + href
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//div[@class="culture_article"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
        detail_text = url_html.xpath('string(//div[@class="culture_article"])').replace('\xa0', '').replace('\n', ''). \
