...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'https://www.ebidding.com/portal/announcement/allTitle?type=10&df=&department=&industry=&bidType=20&guanjianzi=&openMode=&showDateSort=1&platformType=%E6%8B%9B%E6%A0%87%E5%B9%B3%E5%8F%B0&_=1680246553039&on=false&page={}', #招标公告
                    ]
    ...
    def parse(self):
        ...
        while True:
            ...
            for li in detail:
                ...
                code = li["code"]
                etId = li["etId"]
                htmlContentId = li["htmlContentId"]
                if etId == None:
                    etId = li["snID"]
                co = quote(code)
                url = f'https://www.ebidding.com/portal/announcement/ebd/{co}/{etId}/{htmlContentId}?_={timep}'.replace(
                    '/None', '')
                # print(title, url, date_Today)
                # time.sleep(666)
                # self.parse_detile(title, url, date_Today,etId,co,htmlContentId,timep)
    ...
    def parse_detile(self, title, url, date,etId,co,htmlContentId,timep):
        print(url)
        ...
        # print(t)
        # time.sleep(2222)
        detail = url_html["result"]["htmlContent"]
        index_url=f'https://www.ebidding.com/portal/html/index.html#page=main:notice_details?&tenderType=2&etId={etId}&type={co}&htmlContentId={htmlContentId}&platform={co}&_={timep}'
