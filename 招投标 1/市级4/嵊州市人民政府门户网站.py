...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'http://www.szzj.gov.cn/col/col1677264/index.html?uid=4774365&pageNum={}', #工程招标
           ]
    ...
    def parse(self):
        ...
        while True:
            ...
            # print(11, text)
            # time.sleep(6666)
            detail_rule = re.compile(r'class="xxlb_xhx">.*?title="(?P<title>.*?)" href="(?P<href>.*?)"  target="_blank.*?class="xxlb_xhx">(?P<date>.*?)</td>          </tr>]]></record>',re.S)
            detail = re.finditer(detail_rule, text)
            for li in detail:
                title = li.group('title')
                date_Today = li.group('date')
                href = li.group('href')
                url = 'http://www.szzj.gov.cn' + href
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//div[@class="bt_content"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
        detail_text = url_html.xpath('string(//div[@class="bt_content"])').replace('\xa0', '').replace('\n', '').\
