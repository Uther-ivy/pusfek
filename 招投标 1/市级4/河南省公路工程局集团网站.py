...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'http://www.gcjjt.com/html/1559//65/80/list{}.html', #招标公告
                    ]
        self.url1='http://www.gcjjt.com/html/1559//65/80/index.html'
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//div[@class="Custom_UnionStyle"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
        detail_text = url_html.xpath('string(//div[@class="Custom_UnionStyle"])').replace('\xa0', '').replace('\n', ''). \
