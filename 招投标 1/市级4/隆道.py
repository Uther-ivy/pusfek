...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'https://www.longdaoyun.com/business/gczb.html', #招标公告
                    ]
    ...
    def parse(self):
        ...
        while True:
            ...
            data = {'currentPage': page,
                    'totalRows': '1485536',
                    'pageSize': 20,
                    'projectProperty': 'gczb'}
            text = tool.requests_post(url=self.url,data=data, headers=self.headers)
            print('*' * 20, page, '*' * 20)
            html = HTML(text)
            # print(11, text)
            # time.sleep(6666)
            detail=html.xpath('//li[@class="bg-ffffff clearfix"]')
            for li in detail:
                title = li.xpath('./p[1]/a/span[2]/@title')[0]
                date_Today = li.xpath('./p[3]/text()')[0]
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//div[@class="longdao-content mg-t20 "]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
        detail_text = url_html.xpath('string(//div[@class="longdao-content mg-t20 "])').replace('\xa0', '').replace('\n', '').\
