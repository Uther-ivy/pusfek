...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'https://cq.tobacco.gov.cn/c/4/{}', #招标公告
                    ]
    ...
    def parse(self):
        ...
        while True:
            ...
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath('//ul[@class="ul-news"]/li[not(@class="line")]')
            for li in detail[1:]:
                title = li.xpath('./a/@title')[0]
                date_Today = li.xpath('./span/text()')[0]
                href = li.xpath('./a/@href')[0]
                url = "https://cq.tobacco.gov.cn" + href
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//div[@class="clearfix"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
        detail_text = url_html.xpath('string(//div[@class="clearfix"])').replace('\xa0', '').replace('\n', ''). \
