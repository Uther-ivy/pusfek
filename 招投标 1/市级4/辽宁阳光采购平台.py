...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'http://www.lnygcg.com/ygcg/procurement/notice/getList?timeStamp={}', #招标公告
                    ]
    ...
    def parse(self):
        ...
        while True:
            page += 1
            data = {"projectType": "", "status": "", "caigouFs": "", "page": page, "clientAjaxClick": 2}
            timep = int(time.time() * 1000)
            text = tool.requests_post_to(url=self.url.format(timep),data=data, headers=self.headers)
            print('*' * 20, page, '*' * 20)
            ...
            for li in detail:
                title = li["gonggaoTitle"]
                date_Today = li["baomingEndDate"].split(' ')[0]
                id = li["id"]
                url = f'http://www.lnygcg.com/purchase/detail/{id}'
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//div[@class="box"]')[0]
