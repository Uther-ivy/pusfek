...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'http://www.yuhang.gov.cn/col/col1229165993/index.html?uid=6101313&pageNum={}', #工程招标
           ]
    ...
    def parse(self):
        ...
        while True:
            ...
            # print(11, text)
            # time.sleep(6666)
            detail_rule = re.compile(r'<record><!.*?<li><a href="(?P<href>.*?)" target="_blank" title="(?P<title>.*?)"><span>.*?</span><i>(?P<date>.*?)</i></a></li>]]></record>',re.S)
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//div[@class="article-panel"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
        detail_text = url_html.xpath('string(//div[@class="article-panel"])').replace('\xa0', '').replace('\n', '').\
        ...
        item['resource'] = '杭州余杭门户网站公共资源交易中心'
        item['shi'] = 6507
        item['sheng'] = 6500
        item['removal']= title
        # print(item["body"])
