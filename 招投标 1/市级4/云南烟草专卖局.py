...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'https://www.yn-tobacco.com/zwgk/gsgg/zbxx/index.html',
            'https://www.yn-tobacco.com/zwgk/gsgg/zbxx/index_{}.html', #招标公告
                    ]
    ...
    def parse(self):
        ...
        while True:
            ...
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath('//div[@class="lb-con-d"]/ul/li')
            for li in detail:
                title = li.xpath('./a/@title')[0]
                date_Today = li.xpath('./span/text()')[0]
                href = li.xpath('./a/@href')[0][1:]
                url = 'https://www.yn-tobacco.com/zwgk/gsgg/zbxx' + href
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//div[@class="nr-con-c"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
        detail_text = url_html.xpath('string(//div[@class="nr-con-c"])').replace('\xa0', '').replace('\n', ''). \
