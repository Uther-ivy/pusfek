...
class baoshan_ggzy:
    def __init__(self):
        self.url_list = [
            'http://zzggzy.zhengzhou.gov.cn/jsgc/004001/{}.html',
        ]
        self.url1='http://zzggzy.zhengzhou.gov.cn/jsgc/004001/subpage.html'
    ...
    def parse(self):
        ...
        while True:
            ...
            print('-' * 50, page)
            html = HTML(text)
            detail = html.xpath('//div[@class="ewb-r-bd"]/ul/li')
            for li in detail:
                title = li.xpath('./div/a/text()')[0].replace('\r', '').replace('\t', '').replace('\n', '').replace(' ',
                                                                                                                    '')
                href = li.xpath('./div/a/@href')[0]
                date_Today = li.xpath('./span[1]/text()')[0].replace('\r', '').replace('\t', '').replace('\n',
                                                                                                         '').replace(
                    ' ', '')
                url = 'http://zzggzy.zhengzhou.gov.cn' + href
                if '发布' in date_Today:
                    continue
                if '测试' in title:
                    continue
    ...
    def parse_detile(self, title, url, date):
        ...
        # try:
        detail = url_html.xpath('//div[@class="xiangxiyekuang"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace('\xa0', '').replace('\ufeff', '')
        detail_text = ''.join(url_html.xpath('//div[@class="xiangxiyekuang"]//text()')) \
            .replace('\xa0', '').replace('\n', '').replace('\r', '').replace('\t', '') \
