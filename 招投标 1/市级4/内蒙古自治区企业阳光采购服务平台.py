...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'http://nmgygcg.ejy365.com/sitenmyg/information/list?type=csgl&stype=csfwgg&page={}', #招标公告
                    ]
    ...
    def parse(self):
        ...
        while True:
            ...
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath('//ul[@class="clearfix changsuo-list five caigouren"]/li')
            for li in detail[1:]:
                title = li.xpath('./div[2]/a/@title')[0]
                date_Today = li.xpath('./div[3]/text()')[0].replace('\n', '').strip(' ')
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//div[@class="nr1"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
        detail_text = url_html.xpath('string(//div[@class="nr1"])').replace('\xa0', '').replace('\n', '').\
        ...
        item['resource'] = '内蒙古自治区企业阳光采购服务平台'
        item['shi'] = 6507
        item['sheng'] = 6500
        item['removal']= title
        # print(item["body"])
