...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'http://114.67.203.23/ynweb/zbgg/005001/?Paging={}', #工程招标
           ]
    ...
    def parse(self):
        ...
        while True:
            ...
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath('//div[@class="contenttop right_top"]//tr')
            for li in detail:
                title = li.xpath('./td[2]/a/@title')
                if title:
                    title = title[0]
                else:
                    continue
    ...
    def parse_detile(self, title, url, date):
        print(url)
        t = tool.requests_get_GBK(url=url,headers=self.headers)
        # print(t)
        # time.sleep(2222)
        url_html = etree.HTML(t)
        detail = url_html.xpath('//div[@class="detailCont"]|//div[@class="content bg2"]')[0]
