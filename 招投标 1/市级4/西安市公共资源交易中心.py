...
class baoshan_ggzy:
    def __init__(self):
        self.url_list = [
            'http://new.xacin.com.cn:7092/queryContent_{}-jyxx.jspx'
        ]
    ...
    def parse(self):
        ...
        while True:
            ...
            # print(11, text)
            # time.sleep(6666)
            detail=html.xpath('//ul[@id="list-color"]/li')
            for li in detail:
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//div[@class="content-main"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace('浏览次数：', '').replace('原文链接', '')
        detail_text = url_html.xpath('string(//div[@class="content-main"])').replace('\xa0', '').replace('\n', ''). \
