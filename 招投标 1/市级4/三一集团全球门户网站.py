...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'https://igsp-api.sany.com.cn/src/purchaser/v1/project-annos/site/projectAnno?orderColumn=0&page={}&projectType=1&size=10&type=1', #招标公告
                    ]
    ...
    def parse(self):
        ...
        while True:
            ...
            for li in detail:
                title = li["title"]
                index_title=quote(title)
                date_Today = li["publishTime"].split(' ')[0]
                id = li["id"]
                publishTime=li["publishTime"].replace(' ','&20')
                url = f'https://igsp-api.sany.com.cn/src/purchaser/v1/project-annos/site/projectAnnoContent?annoId={id}&titleIcon=sourcNoticeDetial'
                # print(title, url, date_Today)
                # time.sleep(666)
                # self.parse_detile(title, url, date_Today,noticeCode)
                if tool.Transformation(date) <= tool.Transformation(date_Today):
                    if tool.removal(title, date):
                        self.parse_detile(title, url, date_Today,publishTime,id,index_title)
                    else:
                        print('【existence】', url)
                        continue
                else:
    ...
    def parse_detile(self, title, url,date,publishTime,id,index_title):
        print(url)
        ...
        detail_text =url_html["content"]
        index_url=f'https://igsp.sany.com.cn/public/noticeDetialPage?r=0.8306252645357299&title={index_title}&id={id}&publishTime={publishTime}&titleIcon=sourcNoticeDetial'
        # print(index_url)
