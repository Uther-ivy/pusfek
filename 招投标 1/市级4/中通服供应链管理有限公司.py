...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'https://zb.chinaccsscm.cn/zbgg/index_{}.jhtml', #招标公告
                    ]
    ...
    def parse(self):
        ...
        while True:
            ...
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath('//div[@class="List2 Top5"]/ul/li')
            for li in detail:
                title = li.xpath('./a/@title')[0]
                date_Today = "".join(li.xpath('./p/span[3]/text()')).split(" ")[0].replace('发布时间：', '')
                href = li.xpath('./a/@href')[0]
                url= 'https://zb.chinaccsscm.cn' + href
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//div[@class="Section1"]|//div[@class="WordSection1"]|//div[@class="Content"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
        detail_text = url_html.xpath('string(//div[@class="Section1"]|//div[@class="WordSection1"]|//div[@class="Content"])').replace('\xa0', '').replace('\n', '').\
