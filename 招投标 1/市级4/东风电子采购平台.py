...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'https://www.dfmjyzx.com/portal/project_notice_info/list', #招标公告
                    ]
    ...
    def parse(self):
        ...
        while True:
            ...
            for li in detail:
                title = li["title"]
                date_Today = li["pulishDate"].split(' ')[0]
                id = li["id"]
                originalId = li["originalId"]
                url = f"https://www.dfmjyzx.com/portal/central/project_notice_info/detail?id={id}&originalId={originalId}"
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//div[@class="info_content"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
        detail_text = url_html.xpath('string(//div[@class="info_content"])').replace('\xa0', '').replace('\n', '').\
