...
def get_project(url):
    ...
    # now_time = '2021-07-21'
    for ti in titles:
        title = ti.xpath('./a/@title')
        info_url = 'https://jtt.xizang.gov.cn/xxgk/fdzdgknr/zbgg' + (ti.xpath('./a/@href')[0]).split('.')[1]+'.html'
        zhao_time = ti.xpath('./span/text()')[0].replace(' ','').replace('\n','')
...
def get_zhaobiao_info(zhao_time, info_url, title):
    print(info_url)
    resp = tool.requests_get(info_url, headers)
    sel = HTML(resp)
    item = {}
    item['zhao_time'] = zhao_time
    item['title'] = title
    item['info_url'] = info_url
    item['body'] = sel.xpath('//div[@class="view TRS_UEDITOR trs_paper_default trs_word"]')
    detail = sel.xpath('string(//div[@class="view TRS_UEDITOR trs_paper_default trs_word"])')
