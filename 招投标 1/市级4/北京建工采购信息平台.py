...
class wenshan_ggzy:
    def __init__(self):
        self.url_list = [
            'https://jc.bcegc.com/portal/list?chnlcode=result&objtype=1&kw=&pageIndex={}&pagestr=1&pageSize=20', #招标公告
                    ]
    ...
    def parse(self):
        ...
        while True:
            ...
            # print(11, text)
            # time.sleep(6666)
            detail = html.xpath('//ul[@class="tbody gdt_hide"]/li')
            for li in detail[1:]:
                title = li.xpath('./div[1]/a/@title')[0]
                date_Today = li.xpath('./div[3]/h3/@title')[0].split(' ')[0]
                href = li.xpath('./@data-href')[0]
                url = 'https://jc.bcegc.com' + href
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//div[@class="content rel"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
        detail_text = url_html.xpath('string(//div[@class="content rel"])').replace('\xa0', '').replace('\n', ''). \
