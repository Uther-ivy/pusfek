...
class wenshan_ggzy:
    def __init__(self):
        self.domain_name = 'https://www.cdt-ec.com'
        self.url_list = [
            'https://www.cdt-ec.com/notice/moreController/getList'
        ]
    ...
    def parse(self):
        ...
        while True:
            ...
            text = requests.session().post(self.url,params=data,headers=self.headers).content.decode()
            print('*' * 20, page, '*' * 20)
            print(11, text.replace('\ufffd', ' ').replace('\u30fb', ' '))
            time.sleep(6666)
            detail = json.loads(text)['data']
            for li in detail:
                title = li['message_title']
                id= li['id']
                url = f'https://www.cdt-ec.com/notice/moreController/moreall?id={id}'
                date_Today = li['publish_time']
                if 'http' not in url:
