...
class wenshan_ggzy:
    ...
    def parse(self):
        ...
        while True:
            ...
            for li in detail:
                ...
                url = 'http://ggzy.yunfu.gov.cn' + li.xpath('./div/a/@href')[0]
    ...
    def parse_detile(self, title, url, date):
        ...
        try:
            detail = url_html.xpath('//*[@id="ZBGGContent"]')[0]
            detail_html = etree.tostring(detail, method='HTML')
            detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
            detail_text = url_html.xpath('string(//*[@id="ZBGGContent"])').replace('\xa0', '').replace('\n', '').\
                replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
    ...
    def get_nativeplace(self, addr):
        city = ''
        city_list = [['10021.001', '云城区'], ['10021.002', '新兴县'], ['10021.003', '郁南县'], ['10021.004', '云安县'], ['10021.005', '罗定']]
