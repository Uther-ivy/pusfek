...
class wenshan_ggzy:
    ...
    def parse(self):
        ...
        while True:
            page += 1
            if page == 1:
                text = tool.requests_get(self.url.format('moreinfo_listjy'), self.headers)
            else:
            ...
            # print(11, text)
            # time.sleep(6666)
            code = self.url.replace('http://whsggzy.wuhu.gov.cn/jyxx/', '').replace('005001/', '').replace('/{}.html', '').replace('005002/', '')
            detail = html.xpath('//*[@id="list{}body"]/tr'.format(code))
            for li in detail:
                title = li.xpath('./td[2]/a/text()')[0]
                url = 'http://whsggzy.wuhu.gov.cn' + li.xpath('./td[2]/a/@href')[0]
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//*[@class="article-info"]/div')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode())
        detail_text = url_html.xpath('string(//*[@class="article-info"]/div)').replace('\xa0', '').replace('\n', '').\
