...
class wenshan_ggzy:
    def __init__(self):
        self.domain_name = 'http://www.xszbjyw.com'
        self.url_list = [
    ...
    def parse(self):
        ...
        while True:
            ...
            for li in detail:
                ...
                url = li.xpath('./td[3]/div/a/@href')[0]
                date_Today = li.xpath('./td[5]/text()')[0].replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '') \
                    .replace('[', '').replace(']', '').replace('/', '-')
                if 'http' not in url:
                    url = 'http://www.xszbjyw.com/Bulletin/' + url
    ...
    def parse_detile(self, title, url, date):
        ...
        try:
            detail = url_html.xpath('//*[@id="PageMain"]/div[2]/div[2]/span/div')[0]
            detail_html = etree.tostring(detail, method='HTML')
            detail_html = html.unescape(detail_html.decode()).replace("display: none", '').replace("\u2022", '').replace('\xa0', '')
            detail_text = url_html.xpath('string(//*[@id="PageMain"]/div[2]/div[2]/span/div)').replace('\xa0', '').replace('\n',
                                                                                                                 ''). \
                replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
            if len(detail_html) < 200:
                int('a')
        except:
            detail = url_html.xpath('//*[@id="PageMain"]/div[2]/div[2]')[0]
            detail_html = etree.tostring(detail, method='HTML')
            detail_html = html.unescape(detail_html.decode()).replace("display: none", '').replace("\u2022",
                                                                                                   '').replace('\xa0',
                                                                                                               '')
            detail_text = url_html.xpath('string(//*[@id="PageMain"]/div[2]/div[2])').replace('\xa0',
                                                                                                       '').replace('\n',
                                                                                                                   ''). \
                replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
            if len(detail_html) < 200:
