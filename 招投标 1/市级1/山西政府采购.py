...
class xinyang_ggzy:
    def __init__(self):
        self.url_list = ['ZcyAnnouncement1', "ZcyAnnouncement2", "ZcyAnnouncement3"]
    ...
    def parse(self):
        ...
        # date = '2021-06-17'
        page = 0
        url_to = 'http://www.ccgp-shanxi.gov.cn/front/search/category'
        while True:
            page += 1
            data = {"categoryCode": self.url, "pageSize": 15, "pageNo": page}
            ...
            print('*' * 20, page, '*' * 20)
            for li in detail:
                url = 'http://www.ccgp-shanxi.gov.cn' + li['_source']['url']
                title = li['_source']['title']
                date_Today = int(str(li['_source']['publishDate'])[:-3])
                # print(title, url, tool.Time_stamp_to_date(date_Today))
                # time.sleep(666)
                if '测试' in title:
                    continue
                if tool.Transformation(date) >= date_Today:
                    ...
                    print('日期不符, 正在切换类型', tool.Time_stamp_to_date(date_Today), self.url)
                    self.url = self.url_list.pop(0)
                    page = 0
    ...
    def parse_detile(self, title, url, date):
        ...
        r = re.findall('name="articleDetail" value="(.*?)<div class="inner-container">', t, re.S)[0]\
            .replace('&quot;', '"').replace('&lt;', '<').replace('&#x3D;', '=').replace('&gt;', '>')\
            .replace('&amp;nbsp;', '')
        r = json.loads(r[:-4])
        detail_html = r['content']
