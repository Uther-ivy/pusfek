...
class xinyang_ggzy:
    ...
    def parse(self):
        ...
        while True:
            ...
            for li in detail:
                ...
                url = li_.xpath('//li/a/@href')[0]
                if 'http' not in url:
                    url = 'http://ggzyjyzx.shandong.gov.cn' + url
                if '测试' in title:
                    continue
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//*[@id="infocont"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace('\xa0', '').replace('\ufeff', '')
        detail_text = url_html.xpath('string(//*[@id="infocont"])') \
            .replace('\xa0', '').replace('\n', '').replace('\r', '').replace('\t', '') \
