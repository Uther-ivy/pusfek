...
class baoshan_ggzy:
    def __init__(self):
        self.url_list = [
            'http://www.weain.mil.cn/api/front/list/cggg/list?LMID=1149231276155707394&&purchaseType=%E4%B8%AD%E6%A0%87%E5%85%AC%E5%91%8A&pageNo=',
        ]
    ...
    def parse(self):
        ...
        while True:
            ...
            if page == 10:
                ...
                page = 0
            print('-'*50, page)
            text = tool.requests_get(self.url+f'{page}',self.headers)
            detail=json.loads(text)
            print(detail)
            for li in detail['list']['contentList']:
                url = li['pcUrl']
                title = li['nonSecretTitle']
                date_Today = li['publishTime'].split(" ")[0]
                if '发布' in date_Today:
                    continue
                if '测试' in title:
                    continue
                url="http://www.weain.mil.cn"+url
