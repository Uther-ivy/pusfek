...
class wenshan_ggzy:
    def __init__(self):
        self.domain_name = 'https://www.shantou.gov.cn'
        self.url_list = [
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath('//*[@id="zoomcon"]')[0]
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
        detail_text = url_html.xpath('string(//*[@id="zoomcon"])').replace('\xa0', '').replace('\n',
                                                                                                             ''). \
            replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
    ...
    def get_nativeplace(self, addr):
        city = ''
        city_list = [['10005.001', '龙湖区'], ['10005.002', '金平区'], ['10005.003', '濠江区'], ['10005.004', '潮阳区'], ['10005.005', '潮南区'], ['10005.006', '澄海区'], ['10005.007', '南澳县']]
