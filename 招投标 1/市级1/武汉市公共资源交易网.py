...
def get_zhaobiaoinfo(info_url,title,data):
    print(info_url)
    ...
    item['body'] = sel.xpath('//div[@class="div-article2"]').extract_first()
    look = re.sub(r'<[^<]+?>', '', item['body'])
    item['detail'] = look.replace('\n', '').replace("\t", '').replace("\r", '').replace('\xa0', '').replace('\xc2','').replace(' ', '')
    item['mid'] = 1403
    ...
    item['resource'] = "武汉市公共资源交易平台"
    save_db(item)

def get_zhaobiaoyushen(url):
    print(url)
    now_time = tool.date
    rst = tool.requests_get(url, headers)
    detail = HTML(rst).xpath('//*[@class="fw-ej-type1 fw-ej-type1-1"]/li')
    for aa in detail:
        title = aa.xpath('string(./p)').strip()
        # print(info_url)
        info_url = aa.xpath('./div[2]/@onclick')[0].replace("window.location=",'').replace("'",'')
        data = aa.xpath('./div[1]/p[3]/span[2]/text()')[0]
        if tool.Transformation(now_time) == tool.Transformation(data):
            if tool.removal(title, now_time):
                get_zhaobiaoinfo(info_url,title,data)
            else:
                print('已存在', title)
        else:
            print('日期不符', data)
...
if __name__ == '__main__':
    ...
    url_ls = [
        'http://ggzyfw.wuhan.gov.cn/whggzy/gczb/index{}.jhtml',
        'http://ggzyfw.wuhan.gov.cn/whggzy/jygkzfcg/index{}.jhtml'
    ]
    for url in url_ls:
        for i in range(1,6):
            if i == 1:
                get_zhaobiaoyushen(url.format(''))
            else:
                get_zhaobiaoyushen(url.format('_'+str(i)))