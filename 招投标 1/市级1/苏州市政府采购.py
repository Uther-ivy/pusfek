...
class wenshan_ggzy:
    def __init__(self):
        self.domain_name = 'https://czju.suzhou.gov.cn'
        self.url_list = [
            'https://czju.suzhou.gov.cn/zfcg/content/searchContents.action'
        ]
    ...
    def parse(self):
        ...
        while True:
            ...
            for li in detail:
                title = li['TITLE']
                #https://czju.suzhou.gov.cn/zfcg/html/project/019f691af12f449494821b23c5605349.shtml
                url = 'https://czju.suzhou.gov.cn/zfcg/html/project/{}.shtml'.format(li['PROJECTID'])
                date_Today = li['RELEASE_TIME'][:10].replace('\xa0', '').replace('\n', ''). \
    ...
    def parse_detile(self, title, url, date):
        ...
        try:
            ...
            detail_html = html.unescape(detail_html.decode()).replace("display: none", '').replace("\u2022",
                                                                                                   '').replace('\xa0',
                                                                                                               '')
            detail_text = url_html.xpath('string(//*[@class="WordSection1"])').replace('\xa0', '').replace('\n',
                                                                                                           ''). \
                replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
        except:
            detail = url_html.xpath('//*[@class="Article"]')[0]
            detail_html = etree.tostring(detail, method='HTML')
            detail_html = html.unescape(detail_html.decode()).replace("display: none", '').replace("\u2022",
                                                                                                   '').replace(
                '\xa0',
                '')
            detail_text = url_html.xpath('string(//*[@class="Article"])').replace('\xa0', '').replace('\n',
                                                                                                           ''). \
                replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
