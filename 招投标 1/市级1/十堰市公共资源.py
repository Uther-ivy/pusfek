...
def get_project(url):
    ...
    resp.encoding='uft8'
    sel = Selector(resp)
    titles = sel.xpath('//div[@class="indexmain"]/div[2]/div[1]/div')
    now_time = tool.date
    # now_time = '2021-07-26'
    for ti in titles:
        title = ti.xpath('./ul/li[1]/h2/a/text()').extract_first()
        info_url = ti.xpath('./@onclick').extract_first().replace("window.location.href='","").replace("'","")
        zhao_time = ti.xpath('./ul/li[3]/span/text()').extract_first().replace("日期：", "").replace("年","-").replace("月", "-").replace("日", "")
...
if __name__ == '__main__':
    ...
    url_list = [
        "http://ggzyjy.shiyan.gov.cn/jyxx/gcjs/zbgg/",
        "http://ggzyjy.shiyan.gov.cn/jyxx/gcjs/cqgg/index.shtml",
        "http://ggzyjy.shiyan.gov.cn/jyxx/gcjs/pbjggs/index.shtml",
        "http://ggzyjy.shiyan.gov.cn/jyxx/gcjs/zbjggs/index.shtml"
    ]
    for url in url_list:
        get_project(url)