...
class wenshan_ggzy:
    def __init__(self):
        self.domain_name = 'http://ggzyjy.tianshui.gov.cn'
        self.url_list = ['A01','A99','A02','A03', 'D', 'C']
    ...
    def parse_detile(self, title, url, date):
        ...
        try:
            ...
            detail = url_html.xpath('//*[@class="jxTradingMainLeft"]')[0]
            detail_html = etree.tostring(detail, method='HTML')
            detail_html = html.unescape(detail_html.decode()).replace("display: none", '')
            detail_text = url_html.xpath('string(//*[@class="jxTradingMainLeft"])').replace('\xa0', '').replace('\n',
                                                                                                              ''). \
                replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
            if len(detail_html) < 200:
                int('a')
    ...
    def get_nativeplace(self, addr):
        city = ''
        city_list = [['14505.001', '秦城区'], ['14505.002', '北道区'], ['14505.003', '清水县'], ['14505.004', '秦安县'], ['14505.005', '甘谷县'], ['14505.006', '武山县'], ['14505.007', '张家川回族自治县']]
