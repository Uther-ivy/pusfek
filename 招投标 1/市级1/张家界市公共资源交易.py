...
class baoshan_ggzy:
    def __init__(self):
        self.url_list = [
            #https://www.hnsggzy.com/jygk/index_2.jhtml
            'https://zhangjiajie.hnsggzy.com/gczb/index.jhtml',
            'https://zhangjiajie.hnsggzy.com/jygkzfcg/index.jhtml',
            'https://zhangjiajie.hnsggzy.com/jygktd/index.jhtml',
            'https://zhangjiajie.hnsggzy.com/cqjy/index.jhtml',
            'https://zhangjiajie.hnsggzy.com/jygkqt/index.jhtml',
        ]
    ...
    def parse_detile(self, title, url, date):
        ...
        try:
            ...
            try:
                ...
                detail_html = etree.tostring(detail, method='HTML')
                detail_html = html.unescape(detail_html.decode())
                detail_text = url_html.xpath('string(//*[@class="div-article2"])').replace('\xa0', '').replace('\n', ''). \
                    replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
            except:
                try:
                    detail = url_html.xpath('/html/body/div[4]/div[2]/div[3]/div[2]')[0]
                    # print(detail)
                    detail_html = etree.tostring(detail, method='HTML')
                    detail_html = html.unescape(detail_html.decode())
                    detail_text = url_html.xpath('string(/html/body/div[4]/div[2]/div[3]/div[2])').replace('\xa0', '').replace('\n',
                                                                                                                               ''). \
                        replace('\r', '').replace('\t', '').replace(' ', '').replace('\xa5', '')
                except:
                    return
