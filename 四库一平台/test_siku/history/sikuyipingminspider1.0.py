...
class MinSpider(object):
    ...
    def get_person_info(self, cid):
        ...
        for data in person_data:
            ...
            for page in range(math.ceil(count / 15)):
                ...
                ps_thread=threading.Thread(target=self.get_certificate,args=(cid,code,page,perlist))
        ...
        try:
            for page in range(math.ceil(project / 15)):
                ...
                for prodict in resdata:
                    pid = prodict['prjNum']
                    pj_thread = threading.Thread(target=self.get_project_data, args=(pid,projectlist,ptype))
                    pj_thread.start()
                    pj_thread.join()
        except Exception as e:
    ...
    def get_project_data(self,pid,projectlist,ptype):
    ...
    # 建造师等级及人员
    def get_certificate(self, cid, code,page, perlist):
    ...
    def get_company_id(self,companyname,fil):
        ...
        # print(resdata)
        if len(resdata) > 0:
            self.cid_list.append(resdata[0]['id'])
        else:
    ...
    def companysearch(self,cid,fil,name):
            try:
                ...
                badcredit = self.get_badcredit(cid)
                goodcredit = self.get_goodcredit(cid)
                black = self.get_black(cid)
                punishlist = self.get_punishlist(cid)
                chage = self.get_change(cid)
                get_detail_info = self.get_detail_info(cid)
                ...
                if personnum > 0:
                    company_data['regporson'] = self.get_person_info(cid)
                if projectnum > 0:
                    company_data['project'] = self.get_project_info(cid, projectnum)
