...
class baoshan_ggzy:
    ...
    def parse(self):
        ...
        while True:
            ...
            # print(text)
            if '<!DOCTYPE html>' in text or (text == ''):
    ...
    def parse_detile(self, title, url, date):
        ...
        detail = url_html.xpath("//div[@id='mainContent']")[0]
        # print(22, t)
        detail_html = etree.tostring(detail, method='HTML')
        detail_html = html.unescape(detail_html.decode()).replace('浏览次数：', '').replace('原文链接', '')
        detail_text = url_html.xpath("string(//div[@id='mainContent'])").replace('\xa0', '').replace('\n', ''). \
