        ...
        try:
            ...
            if content:
                ...
                for num in range(1,len(size)+1):
                    ...
                    if require:
                        require = require[0]
                    price = detail.xpath(f"//tbody/tr[not(@class='firstRow')][{num}]/td[4]//text()")
                    if price :
                        price =price[0]
                    futher = detail.xpath(f"//tbody/tr[not(@class='firstRow')][{num}]/td[5]//text()")
                    if futher:
                        futher=futher[0]
                        if '年' in futher:
...
def run(page,spider,times,file):
    for param in ['cgyx_2', 'cgyx_3', 'cgyx_4']:
        for lis in spider.get_data_list(param,times,page):
            ...
            prodict['detail'] = spider.get_data_detail(param,articleId)
