...
class cgyxspider(object):
    ...
    def get_data_list(self, times,page):
        url = 'https://www.ccgp-neimenggu.gov.cn/category/cgysgk'
        payload = None
        data={
            'keyword':'',
            'page': page,
        ...
        all_data = html.xpath("//ul[@id='itemContainer']")
        for num in range(1, len(all_data) + 1):
            auditTime = html.xpath(f"//li[{num}]/span/text()")
            ...
            if releasetime >= todaytime:
                title = html.xpath(f"//li[{num}]//span[@class='cgysgk_title']/text()")[0]
                href = html.xpath(f"//ul[@id='itemContainer']/li[{num}]/a/@href")[0]
