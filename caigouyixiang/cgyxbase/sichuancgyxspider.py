...
class cgyxspider(object):
    ...
    def get_data_detail(self, url):
        ...
        if res:
            ...
            content= etree.tostring(detail.xpath("//table[@class='noticeTable']")[0],method="HTML").decode()
            # for num in range(2,len(size)+1):
            #     detail_dict = {}
            #     proname = detail.xpath(f"//tr[{num}]/td[2]/text()")
            #     if proname:
            #         proname = proname[0].replace('\u3000','')
            #     require = detail.xpath(f"//tr[{num}]/td[3]/div")
            #     if require:
            #         content = ''
            #         for text in require:
    ...
    def get_data_list(self, times,page):
        url = f'http://www.ccgp-sichuan.gov.cn/freecms/rest/v1/notice/selectInfoMoreChannel.do?&siteId=94c965cc-c55d-4f92-8469-d5875c68bd04&channel=c5bff13f-21ca-4dac-b158-cb40accd3035&currPage={page}&pageSize=10&noticeType=59&regionCode=&purchaseManner=&title=&openTenderCode=&purchaser=&agency=&purchaseNature=&operationStartTime=&operationEndTime=&selectTimeName=noticeTime&cityOrArea='
        print(url)
        payload =None
        data=None
...
def run(page,spider,times,file):
    ...
    for lis in spider.get_data_list(times,page):
        ...
        prodict['title'] = title
        detailurl = f'http://www.ccgp-sichuan.gov.cn/freecms{href}?noticeType=59'
