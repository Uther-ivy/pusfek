...
class cgyxspider(object):
    def __init__(self):
        ...
        'Content-Type': 'application/json',
            'Referer':'http://www.ccgp-xinjiang.gov.cn/luban/category?parentId=3661&childrenCode=ZcyAnnouncement&utm=luban.luban-PC-42055.959-pc-websitegroup-navBar-front.4.a110cfa01c7c11eeb3c02322d835e964',
    ...
    def get_data_list(self, times,page):
        url = 'http://www.ccgp-xinjiang.gov.cn/portal/category'
        print(url)
        payload = {
        ...
        print(data_list)
        if data_list:
            jsondata = json.loads(data_list).get('result').get('data').get('data')
            for lis in jsondata:
                # lis = data['_source']
                ...
                if releasetime >= todaytime:
                    ...
                    title = lis['title']
                    author=lis['author']
                    href = f'http://www.ccgp-xinjiang.gov.cn/portal/detail?articleId={urllib.parse.quote(articleId)}&parentId=3661&timestamp={int(time.time())}'
                    showurl= f'http://www.ccgp-xinjiang.gov.cn/luban/detail?parentId=3661&articleId={articleId}&utm='
                    yield districtName,releasetime,articleId,title,href,showurl,author
                else:
...
def run(page,spider,times,file):
    ...
    for lis in spider.get_data_list(times,page):
        districtName,releasetime,articleId,title,href,showurl,author=lis
