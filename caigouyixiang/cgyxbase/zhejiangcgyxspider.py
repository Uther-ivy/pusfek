...
class cgyxspider(object):
    ...
    def get_data_detail(self, url):
        ...
        if res:
            html=json.loads(res)['noticeContent']
            # print(html)
            detail = etree.HTML(html)
    ...
    def get_data_list(self, times,page):
        url = f'https://zfcgmanager.czt.zj.gov.cn/cms/api/cors/remote/results?pageSize=100&pageNo={page}&sourceAnnouncementType=10016&isGov=true&url=notice'
        print(url)
        payload =None
        data=None
...
def run(page,spider,times,file):
    ...
    for lis in spider.get_data_list(times,page):
        ...
        prodict['articleId'] = articleId[-6:]
        prodict['publishDate'] = releasetime
        prodict['title'] = title
        detailurl = f'https://zfcgmanager.czt.zj.gov.cn/cms/api/cors/remote/results?noticeId={articleId}%26utm&url=noticeDetail'
