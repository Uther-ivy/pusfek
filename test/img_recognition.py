...
if __name__ == '__main__':
    # img_math_recognition('zgsyzbw.png')
    import ddddocr
    ...
    res = ocr.classification(ss)
    print('识别出的验证码为：' + res)
    ss=re.findall(r'(.*)=', res)
    # print(ss[0],math(ss[0]))
    print(eval(ss[0]))
    print(eval('2*5'))
    ...
    # with open('zgsyzbw.png', 'wb') as w:
    #     w.write(base64.b64decode(ss))
    # image = Image.open('zgsyzbw.png')
    # image = image.convert('L')
    # image = image.convert('1')
    # count = 120
    # table = []
